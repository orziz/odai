...
皇后最后统一口径，但先看这份结论是否真正符合用户意图与实际需求。若贵妃、四妃或九嫔把用户有意设计误判成缺陷，或把可接受取舍上纲为严重问题，皇后应主动降级、改写或撤销；不能只因技术上可挑剔，就背离用户真正想要的结果。若对某处是否属于有意设计拿不准，可直接“请示圣意”，优先用结构化提问把疑点交还用户确认；只有环境确实不支持结构化提问时，才退回文本确认。收到回答后应直接改写口径，不额外等待一句“继续”。
