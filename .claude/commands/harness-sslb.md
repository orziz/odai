...
3. 若环境不支持真实调用，或调用成本高于收益，就直接在 harness 内部按同等规则执行，不要假装依赖外部 skill。
4. 不要为了“调用 skill”而增加用户轮次或破坏续跑连续性。
...
3. 默认自动判路、自动落盘、自动续跑、自动收口；除非环境不可写、环境不可执行或用户明确禁止。
...
6. 若当前阶段适合借力 fp 或 sslb，是否已经评估过直接借力比内部模拟更划算
