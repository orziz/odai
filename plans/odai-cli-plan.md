...
- preferences:新增 `.odai/preferences.json` 本机偏好文件,由 `cli/src/core/preferences.mjs` 读写;交互式 session 启动时会恢复 language、默认 provider、默认 model、reasoning depth、context window 和 `/auth` 的 API-key / provider-command 使用确认,用户执行 `/language`、`/provider`、`/model`、`/reasoning`、`/context <...>`、`/auth ...` 成功后自动回写。该文件已加入 `.gitignore`,不作为团队配置提交。preferences 只保存“允许使用”确认,不保存 API key、token、provider session 或 CLI 登录凭证;真实 key 仍在 shell env / `.odai/secrets.env`,订阅 CLI 登录态仍由 provider CLI 自己管理。`/auth clear` 会清除持久化确认。
