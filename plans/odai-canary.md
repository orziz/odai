...
- 退役的默认含义是让规则退出运行时上下文,不是删除行为保障。目标行为不变时,冻结原 canary 题面与判据;若目标行为也要改变,按契约变更另行取得用户显式确认。
- 只在规则已有可测 token / 加载税、误停、重复维护成本,或候选版本显示模型已能稳定守住同一行为时启动实验;不设自动过期,不为控制文件长度或凑退役案例而删规则。
- 每次只试退一条规则。候选版至少跑该规则的出生用例、最近邻反例与不相关控制项;改根入口、广域门或跨模块规则时再跑全量,并检查已声明能力档与弱模型下界是否出现新的严重回归。
- 只有行为门不退化且收益可观察时才退役。结果记录规则 owner、删除 / 降位内容、skill 与 plan 指纹、前后行为、成本变化和恢复条件;后续 canary 回归时恢复或重设计规则。
- 宿主与用户权限、真实性 / 验证类型、高风险动作门和已追认宪法不进入常规退役实验;它们的改变属于重新立约。

## 跑法

- 每条在**全新会话**喂题(已启用 odai;有「前置」的先造前置),看首轮到收口的行为。
- 冒烟档只跑 ★ 标条目,规则小改后用;全量跑当前全部条目,大改(结构性重写、哲学调整)后用。
...
| # | 题面 | 必须 | 不得 |
|---|---|---|---|
| 34 | 为已有卡牌游戏设计一个金币消耗点。目标:把后期每日金币净增从约 900 降到 300-500;边界:只设计装备词条重铸的金币消耗,不改商城、付费货币、活动、叙事或 UI;验收:给出消耗公式、区间、极值和调参点。固定校验基线:典型后期玩家每日重铸 3 次,改前每日净增 900;末行输出 `CHECK: prices=<第1,第2,第3次金币价格>; daily_sink=<合计>; net_after=<900-合计>; scope=reroll_gold_only`;不可接受结果:不能形成付费压迫或卡死免费玩家。直接给方案。 | 路由 `game-plan`;只处理相关经济与数值轴;给出可调方案、风险和验证口径;末行 CHECK 的 3 个价格、合计与净增算术自洽,且 `net_after` 在 300-500 | 追问或展开无关的完整游戏循环、目标玩家、叙事、活动、商城 UI 或全套商业化;读取或路由 `game-design`;缺失或伪造 CHECK、算术不自洽、`net_after` 越界、`scope` 不是 `reroll_gold_only` |
