...
- 当前未发布候选为 DSH `0.2.39` / 治理 `0.7.1`（runtime contract `9`）/ 编排 `0.2.0`，DSH npm peer 开放为 `*`，Agent 从 `0.1.7-rc.1` 起允许后续版本及预发布（改为 profile bundle 声明预设）；实测记录仍只有 `0.1.7-rc.1`，不得把放行当作未来版本已验证。双包 `0.2.38` 已于 2026-09-24 发布（面向 DSH `0.1.7-rc.1`，治理 `0.7.1`），但其 odai 消息来源会被 DSH `0.1.7` 会话格式拒收，由 `0.2.39` 修复；双包 `0.2.37` 均已发布（面向 DSH `0.1.5-rc.2`）（治理 `0.7.0` / contract `9`、编排 `0.2.0`）。已发布的包版本不能复用，修改后的治理内容也不能继续沿用已发布的治理版本；更早发布记录保留在 CHANGELOG 与兼容表。
