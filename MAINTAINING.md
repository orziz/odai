...
- 当前候选为 DSH `0.2.29` / canonical `0.3.11`，仅面向 DSH `0.1.5-rc.1`。`0.2.28` 已发布；`0.2.27` 的 Sol/high C19 冻结定向结果为 4/4，完整交付及相邻修订的 C12/C32 保持项见 [`docs/evaluation-results.md`](docs/evaluation-results.md)。新候选尚无独立模型计分、全量或配对 A/B；`0.3.9` 的历史 18/19、142/144 及 C19 的 3/4 均按原指纹保留，不迁移为新候选成绩。
