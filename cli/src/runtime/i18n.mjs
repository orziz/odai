...
const TRANSLATIONS = {
  en: {
    ...
    "interactive.typeTask":
      "Type a task or /help. Use /provider select, /models select, /auth select, and Tab completion for setup.",
    "interactive.help":
      "Commands: /providers [--json], /provider <name|auto|select>, /provider add|set|remove|clear, /provider path, /models [select|--json|--provider <name>], /model <model|provider:model|auto|select>\nSession: /auth [select|api-key|claude-cli|claude-agent-sdk|provider-command|shell|network|all|clear], /reasoning <auto|none|minimal|low|medium|high>, /context <auto|200k|1m>, /settings, /language <zh|en>\nOperate: /run <task> [--model <model> --reasoning <depth> --context <tokens> --subagent reviewer:auto[:model] --subagent challenger:<provider>[:model] --exclude-provider <name>], /retry, /continue [--run] [--use-api-key] [--use-provider-command] [--allow-shell] [--allow-network]\nInspect: /agents, /doctor, /setup, /status, /audit, /evidence, /policy, /sessions, /rollback, /authorize <scope>, /init, /skills, /exit\nTab completes commands, providers, model labels, and setting values.",
    ...
    "completion.auth.select": "Open auth picker",
    "completion.auth.claudeCli": "Allow only the local Claude CLI provider",
    "completion.auth.claudeAgentSdk": "Allow only the Claude Agent SDK provider",
    "completion.auth.providerCommand": "Allow subscription CLI providers for this session",
    "completion.auth.shell": "Allow shell tools for this session",
    "completion.auth.network": "Allow network tools for this session",
    "completion.auth.all": "Allow API-key and provider-command use",
    "completion.auth.clear": "Clear session auth confirmations",
    "completion.provider.select": "Open provider picker",
    "completion.provider.add": "Add an OpenAI-compatible provider",
    "completion.provider.set": "Create or update a provider",
    "completion.provider.remove": "Remove a configured provider",
    "completion.provider.clear": "Clear a provider API key binding",
    "completion.provider.path": "Show active provider config files",
    "completion.provider.openaiCompatible": "OpenAI-compatible gateway",
    "completion.provider.name": "Provider name",
    "completion.provider.baseUrl": "Gateway base URL",
    "completion.provider.model": "Default model",
    "completion.provider.models": "Comma-separated model hints",
    "completion.provider.apiKeyEnv": "API key environment variable",
    "completion.provider.workspace": "Save to this workspace instead of global config",
    "completion.provider.replace": "Replace an existing provider",
    ...
    "completion.providers.json": "Show provider diagnostics",
  ...
  zh: {
    ...
    "interactive.typeTask":
      "输入任务或 /help。用 /provider select、/models select、/auth select 和 Tab 补全完成设置。",
    "interactive.help":
      "命令: /providers [--json]、/provider <name|auto|select>、/provider add|set|remove|clear、/provider path、/models [select|--json|--provider <name>]、/model <model|provider:model|auto|select>\n会话: /auth [select|api-key|claude-cli|claude-agent-sdk|provider-command|shell|network|all|clear]、/reasoning <auto|none|minimal|low|medium|high>、/context <auto|200k|1m>、/settings、/language <zh|en>\n执行: /run <task> [--model <model> --reasoning <depth> --context <tokens> --subagent reviewer:auto[:model] --subagent challenger:<provider>[:model] --exclude-provider <name>]、/retry、/continue [--run] [--use-api-key] [--use-provider-command] [--allow-shell] [--allow-network]\n查看: /agents、/doctor、/setup、/status、/audit、/evidence、/policy、/sessions、/rollback、/authorize <scope>、/init、/skills、/exit\nTab 会补全命令、provider、模型标签和设置值。",
    ...
    "completion.auth.select": "打开授权选择器",
    "completion.auth.claudeCli": "只允许当前会话使用本机 Claude CLI provider",
    "completion.auth.claudeAgentSdk": "只允许当前会话使用 Claude Agent SDK provider",
    "completion.auth.providerCommand": "允许当前会话使用订阅 CLI provider",
    "completion.auth.shell": "允许当前会话使用 shell 工具",
    "completion.auth.network": "允许当前会话使用 network 工具",
    "completion.auth.all": "允许 API-key 和 provider-command",
    "completion.auth.clear": "清除当前会话授权确认",
    "completion.provider.select": "打开 provider 选择器",
    "completion.provider.add": "添加 OpenAI-compatible provider",
    "completion.provider.set": "创建或更新 provider",
    "completion.provider.remove": "删除已配置 provider",
    "completion.provider.clear": "清除 provider API key 绑定",
    "completion.provider.path": "显示当前 provider 配置文件",
    "completion.provider.openaiCompatible": "OpenAI 兼容中转",
    "completion.provider.name": "Provider 名称",
    "completion.provider.baseUrl": "中转 base URL",
    "completion.provider.model": "默认模型",
    "completion.provider.models": "逗号分隔的模型提示",
    "completion.provider.apiKeyEnv": "API key 环境变量",
    "completion.provider.workspace": "保存到当前项目而不是全局配置",
    "completion.provider.replace": "替换已有 provider",
    ...
    "completion.providers.json": "显示 provider 诊断",
