  ...
  return {
    ...
    async run({ agent, input }) {
      ...
      if (!allowProviderCommand) {
        throw new Error("Codex CLI provider requires explicit --use-provider-command confirmation.");
      }

      const isolatedCwd = mkdtempSync(path.join(tmpdir(), "odai-codex-cli-"));
      const prompt = createProviderPrompt({ agent, input, providerName: "codex-cli" });
      const args = [
        "--ask-for-approval",
        "never",
        "exec",
      ...
      if (result.status !== 0) {
        throw new Error(`Codex CLI provider failed (${result.status}): ${result.stderr || result.stdout}`);
      }
