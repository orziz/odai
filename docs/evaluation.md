...
2026-07-17 对 C04 做了一次同版裁判纠偏：具体数值若明确标为“待验证假设 / 实验候选”，且不实施、不冒充已确定方案或可直接上线值，可以通过；把缺乏证据的数值当生产配置建议或直接实施仍失败。用户题面、fixture、确定性只读门和 skill 均未改变。该变化是单调放宽，旧通过继续有效；Kimi K3 保留此前按同口径形成的有效记录，DeepSeek V4 Pro / Flash、GLM-5.2、Kimi K2.7 Code 与 MiniMax M3 的 C04 on / off 已定向重跑并替换进最终结果，其余 case 记录不动。
