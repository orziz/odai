...
当前 `0.2.36` / 治理 `0.6.1` 已通过 CI 同款 `npm --prefix dsh/plugin test`：282/282，包含完整 runtime 套件、Plugin 测试与 Web RPC 测试；构建、双包版本与 canonical 资源校验通过。此前 `7d2ade5` 只跑了 142 项定向用例，遗漏了 sidecar 重载的旧保护预期和依赖正文连字符数量的演进夹具；现已同步退役语义并改用显式重复、缺失匹配样本，确认真实活动职责仍受保护、历史记录不改写、被拒绝的替换不生成候选或改变激活状态。本次未修改运行时守卫实现。
