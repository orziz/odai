...
当前 `0.2.36` / 治理 `0.6.1` 已通过 `npm run check:dsh && npm --prefix dsh/plugin test`：源码及 runtime、Plugin、Agent 测试的类型检查、客户端语法检查通过；完整 runtime、Plugin 与 Web RPC 测试 282/282 通过。双包版本与 canonical 资源校验通过。此前仅跑定向或 tsx 测试，遗漏了旧保护预期、依赖正文的演进夹具及测试类型收窄错误；均已修正。检查确认活动职责仍受保护、历史记录不改写、被拒绝的替换不生成候选或改变激活状态；这些修正未放宽运行时守卫。收尾检查另通过 Agent 测试 34/34、双包 `pack:dry-run` 及生成目录清理检查。
