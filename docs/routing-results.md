...
已发布 `0.5.0` 的 rc.2 隔离加载、权限、安装与制品记录按原版本保留在 CHANGELOG 和 Git 历史中，不自动转为当前候选验收。当前候选已通过隔离双包共存检查与制品校验，但未重跑完整宿主矩阵。2026-09-21 的 plain 治理定向结果见 [`evaluation-results.md`](evaluation-results.md)：C01、C04、C05、C26、C32 当前均采用 4/4，其中 C04 为用户裁定，其余为独立裁判；不作为编排收益或全量评测证明。
