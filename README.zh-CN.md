...
截至 2026-07-12，冻结版本在 40 条行为 Canary 上取得以下结果：

| Runner | Judge | 结果 | 结论 |
|---|---|---:|---|
| GPT-5.5 / medium | GPT-5.6 Sol / high | 40/40，0 unresolved | 已验证配置；含 4 条纯英文治理迁移案例 |
| GPT-5.4 Mini / low | GPT-5.5 / high | 25/40，0 unresolved | 能完成轻量与明确任务，但不作为完整治理承诺档 |

这些结果描述已实跑配置，不代表模型品牌排名或对未测试宿主的保证。弱模型仍可能漏掉停手门、证据复扫、验收字段或 agent 交接限定；需要完整治理可靠性时，使用与已验证配置相当的强模型与推理档。
