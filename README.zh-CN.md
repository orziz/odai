...
当前 `0.2.39` 源码候选放开 DSH npm peer，Agent 允许 `>=0.1.7-rc.1`（含后续预发布），不维护逐版本白名单；当前仅实测 `0.1.7-rc.1`，未来版本仍须具备相同的 bundle 与运行时接口。已发布的 `0.2.38` 与候选相同的宿主范围，但它的 odai 提醒、记忆包和压缩说明会被 DSH `0.1.7` 拒收并导致整轮失败，请改用 `0.2.39`；已发布的 `0.2.37` Plugin 与 Agent 仅支持精确的 `dsh@0.1.5-rc.2`。DSH `0.1.7` 下 Agent 以 profile bundle 形式安装，由它声明 `odai` 预设。已发布版本的历史记录保留。Odai 自行维护 preset，保留现有能力，不要求照搬 Standard，也不自动添加其新增工具。Control Center 声明新版 Web 传输依赖，非 Web 环境的治理仍独立可用。
...
odai 从理解到交付持续负责当前任务。八份按需资料分别负责边界、计划、制作、验证、状态与记忆、外部能力、日常关怀与危机保护，各自只补当前缺口。
...
| `craft.md` | 已决定结果的实施与排障、设计、UI / 实时交互、文档与审查工艺 |
| `planning.md` | 可执行工程计划、需求覆盖、工作包依赖、持久交接与恢复顺序 |
| `verification.md` | 验收、证据强度与完成判断 |
| `memory.md` | 长期信息、项目叠加层与项目规则和技能的固化；续作状态由 planning 统一维护 |
| `leverage.md` | 工具、资料与技能选择；外部能力的发现、净增益、安装、创建与组合 |
...
skill 负责判断，Hooks 只把项目已经明确的边界变成机械护栏。它们默认不安装、不启用，也不改变 `odai` 的主流程；只有项目创建 `.odai/hooks.json` 后，才会针对受支持的结构化写工具（含 Claude Code 的 `NotebookEdit`）保护显式只读路径，并在收口时运行显式声明且与当前变更相关的验收命令。没有策略文件时完全静默。
...
Grok Build 当前只有 `PreToolUse` 是可阻断边界，因此适配器不会伪造 Stop 验收。Hooks 只检查受支持的结构化写工具和项目明确配置的验收命令；普通 shell 写入不在此路径保护范围内，Hooks 不解析任意 shell 命令，也不推断用户意图、目标文件或测试方案；它们是权限、沙箱和人工确认之外的轻量保险丝，不是完整安全边界。启用前应审查生成配置与 `.odai/hooks.json`。

## 评测

当前源码候选为 DSH `0.2.39` / 治理 `0.7.1`（runtime contract `9`）/ 编排 `0.2.0`，最新已发布 DSH 为 `0.2.38`（治理 `0.7.1`、contract `9`、编排 `0.2.0`，有上述会话格式缺陷），面向 DSH `0.1.5-rc.2` 的已发布版本为 `0.2.37`（治理 `0.7.0`）。当前证据与限制见[模型评测](docs/evaluation-results.md)及[运行时验证](docs/routing-results.md)，历史模型分数不作为后续源码的质量证明。
