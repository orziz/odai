...
当前开发候选为 DSH `0.2.30` / canonical `0.3.12`，没有新采纳的模型成绩或 token 节省实测。已发布的 `0.3.11` 尚无新模型计分或 token 节省实测。`0.3.10` 冻结快照的 GPT-5.6 Sol/high C19 完整能力交付为 4/4，不作为 `0.3.11` 的成绩。完整输出、范围、保持项与指纹见 [`docs/evaluation-results.md`](docs/evaluation-results.md)。`0.3.9` 冻结快照仍为 18/19、142/144，其 C19 历史 3/4 保留；这些分数及下表的更早结果不迁移到当前源码。
