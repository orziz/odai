...
已发布的 `0.2.21` Plugin 与 Agent 支持正式 `dsh@0.1.1-rc.2`，并以 `>=0.1.2-alpha.5 <0.1.2` 接纳 alpha.5 起的同一 prerelease 线；该版本当时实际加载过 rc.2、alpha.5 与 rc.1，source composition 固定为 rc.1。它们修复了已发布 `0.2.20` 的 Control Center client：后者在 alpha.5 或 rc.1 中会等待仅 rc.2 存在的 conversation services，进而阻断 Web boot。当前 `0.2.22` 版本保持同一支持范围，但 release matrix 只跑有区分力的 rc.2 与 rc.1；alpha.5 仍由范围覆盖，不再单列重复检查。`0.2.22` 同时为有界 planner/reviewer 交接增加经用户原文核验的 active/superseded requirement provenance，不增加角色或执行阶段。Plugin 安装命令要求 `pnpm` 已在 `PATH` 中。已发布 `0.2.19` 保留精确的 rc.2 + alpha.4 历史合同，`0.2.18` 则保留更早的 alpha.2 合同以及 Control Center 来源/版本修复、回滚和默认同意确认行为。两个包都已经包含 canonical Odai skill、共享 DSH runtime 与同一份中文 Control Center，不存在第三个包。Plugin 安装后直接提供 Control Center；Agent 的交互式 `install` 会说明现有 profile 的来源和版本，并在 `[Y/n]` 询问；直接回车、`y` 或 `yes` 确认，EOF、`n`、`no` 或其他文本不修改 Web profile。非交互安装必须显式传 `--with-control-center` 才会修改 profile。Agent 完整保留所固定 DSH Standard preset 的全部能力，再以 scoped 扩展叠加 Odai。Plugin 与 Agent 可以独立安装，也允许同时安装；共存时由 Plugin 拥有唯一 Control Center 界面，两套 runtime 共享并去重治理、路由与 evidence 状态。现有 provider-neutral `odai-cli` 继续作为独立产品。
