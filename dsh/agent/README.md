...
The installer checks `dsh -V`; published `0.2.21` and the current `0.2.22` release accept formal `0.1.1-rc.2` or `>=0.1.2-alpha.5 <0.1.2`, while alpha.4, other `0.1.1` previews, and stable `0.1.2` are rejected. The current release matrix runs only the distinct rc.2 and rc.1 contracts; alpha.5 remains covered by the SemVer range and uses the same Standard composition and `uiConversation` contract as rc.1, so it has no separate installer, renderer, or matrix check. The installer records the detected version in the managed manifest, uses the rc.2 legacy renderer for rc.2, and uses the rc.1 source renderer for the accepted `0.1.2` prerelease line. In an interactive terminal it classifies the Control Center profile as absent, current, registry-upgrade, local-link, partial-drift, newer, or unknown-source. Every add, upgrade, replacement, or repair is shown at `[Y/n]`; Enter, `y`, or `yes` confirms, while EOF, `n`, `no`, and other text leave the profile unchanged. Non-interactive and `--json` installs never infer consent: automation can use `--with-control-center` or `--without-control-center` explicitly, with `--profile <name>` selecting a profile other than `web`.
