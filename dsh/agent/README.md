...
The installer checks `dsh -V`; the unpublished `0.2.21` candidate accepts formal `0.1.1-rc.2` or `>=0.1.2-alpha.5 <0.1.2`, while alpha.4, other `0.1.1` previews, and stable `0.1.2` are rejected. rc.2, alpha.5, and rc.1 are the exact release-matrix anchors; later in-range prereleases are accepted under the upstream SemVer contract. npm registry publication remains a separate follow-up step. The installer records the detected version in the managed manifest, uses the rc.2 legacy renderer for rc.2, and uses the rc.1 source renderer for the accepted `0.1.2` prerelease line; alpha.5 is included because its Standard composition and `uiConversation` client contract are identical to rc.1. In an interactive terminal it classifies the Control Center profile as absent, current, registry-upgrade, local-link, partial-drift, newer, or unknown-source. Every add, upgrade, replacement, or repair is shown at `[Y/n]`; Enter, `y`, or `yes` confirms, while EOF, `n`, `no`, and other text leave the profile unchanged. Non-interactive and `--json` installs never infer consent: automation can use `--with-control-center` or `--without-control-center` explicitly, with `--profile <name>` selecting a profile other than `web`.
