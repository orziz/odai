...
Choose one surface for the scope you need. The Plugin manager requires `pnpm` on `PATH`; the `0.2.17` Agent installer candidate supports exactly `dsh@0.1.1-rc.2` and `dsh@0.1.2-alpha.2`.
...
- Agent-only installation contains its canonical skill and runtime and needs no Plugin. Its installer changes a DSH profile only after explicit `y/yes` confirmation at `[y/N]` or an explicit non-interactive flag, solely to expose the Agent package's own Control Center.
