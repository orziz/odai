...
| --- | --- | --- | --- |
| `0.2.27` | `0.2.27` | `0.1.2-rc.1` | 当前未发布候选；canonical 0.3.10 减少普通任务的额外流程，修正默认工艺注入、前端路由、回交和压缩续作；C19 定向交付评测为 4/4，当前全量与配对 A/B 尚未运行 |
...
3. `dshVersions` 声明 release matrix 的精确验证目标，实际通过须有对应候选的运行证据；`dshRange` 声明支持范围。当前 `0.2.27` 的范围与矩阵目标都收敛到 rc.1，新增候选不继承历史版本的实际通过记录；没有 npm 发布制品的 `0.1.3-alpha.1` 不进入矩阵或 peer，即使其官方 tag 能构建且 Standard composition 摘要相同。
