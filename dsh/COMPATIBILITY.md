...
| --- | --- | --- | --- |
| `0.2.37` | `0.2.37` | `0.1.5-rc.2` | 未发布候选；治理 0.7.0 / runtime contract 9；编排 0.2.0；恢复规则账本中有来源的规则，退役技能演进（既有数据不读取、不删除），未配置职责映射时不暴露职责路由工具；保留已发布的 goal / todo 提示边界 |
...
2. `0.2.15` 至 `0.2.18` 只支持精确的 `0.1.1-rc.2` 与 `0.1.2-alpha.2`；`0.2.19` 只支持精确的 rc.2 与 alpha.4。`0.2.20`、`0.2.21` 与 `0.2.22` 按已发布合同保留 rc.2 和后续 `0.1.2` prerelease 范围；`0.2.23` 至 `0.2.27` 只支持精确 `0.1.2-rc.1`。`0.2.28` 至已发布的 `0.2.32` 仅面向精确 `0.1.5-rc.1`；已发布的 `0.2.33`、`0.2.35`、`0.2.36` 与当前候选 `0.2.37` 均仅支持精确 `0.1.5-rc.2`，不保留 rc.1 兼容分支。
