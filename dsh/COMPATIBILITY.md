...
| --- | --- | --- | --- |
| `0.2.33` | `0.2.33` | `0.1.5-rc.2` | 未发布候选；canonical 0.5.0 / runtime contract 7；manifest schema 3，完整入口与受托职责通过统一组合器消费；旧 schema 外部 bundle 明示降级且不改写原数据；仅支持 rc.2，统一职责执行权限与工具展示，子代理移除总控专用输出提示；模型评测按实际冻结指纹另行记录 |
...
2. `0.2.15` 至 `0.2.18` 只支持精确的 `0.1.1-rc.2` 与 `0.1.2-alpha.2`；`0.2.19` 只支持精确的 rc.2 与 alpha.4。`0.2.20`、`0.2.21` 与 `0.2.22` 按已发布合同保留 rc.2 和后续 `0.1.2` prerelease 范围；`0.2.23` 至 `0.2.27` 只支持精确 `0.1.2-rc.1`。`0.2.28` 至已发布的 `0.2.32` 仅面向精确 `0.1.5-rc.1`；当前候选 `0.2.33` 仅支持精确 `0.1.5-rc.2`，不保留 rc.1 兼容分支。
