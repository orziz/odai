...
| --- | --- | --- | --- |
| `0.2.38` | `0.2.38` | peer `*`；Agent ≥ `0.1.7-rc.1`（含预发布）；实测 `0.1.7-rc.1` | 未发布候选；治理 0.7.1 / runtime contract 9；编排 0.2.0；Agent 改为 profile bundle，以 `@deepseek-ai/dsh-agent-preset` 声明 `odai` 预设（宿主不再扫描 `.agent-presets`），工作流引擎改用 `workflow-ptc`，`ralph` 与插件管理工具默认关闭，与 Standard 一致；旧预设目录原样保留并提供移入备份的清理命令；入口按五问组织，审查范围完整时允许原文尾部截断 |
...
2. `0.2.15` 至 `0.2.18` 只支持精确的 `0.1.1-rc.2` 与 `0.1.2-alpha.2`；`0.2.19` 只支持精确的 rc.2 与 alpha.4。`0.2.20`、`0.2.21` 与 `0.2.22` 按已发布合同保留 rc.2 和后续 `0.1.2` prerelease 范围；`0.2.23` 至 `0.2.27` 只支持精确 `0.1.2-rc.1`。`0.2.28` 至已发布的 `0.2.32` 仅面向精确 `0.1.5-rc.1`；已发布的 `0.2.33`、`0.2.35`、`0.2.36`、`0.2.37` 均仅支持精确 `0.1.5-rc.2`，不保留 rc.1 兼容分支；当前候选 `0.2.38` 使用开放 npm peer，Agent 仅保留 `0.1.7-rc.1` 最低门槛并允许后续预发布，不保留旧安装协议分支；当前实测仅 `0.1.7-rc.1`，更高版本仍依赖宿主接口兼容。
