...
| --- | --- | --- | --- |
| `0.2.26` | `0.2.26` | `0.1.2-rc.1` | 当前未发布候选；canonical 0.3.9 保留精神内核与五项，澄清授权、支撑与协作合同；rc.1 实际压缩包的隔离安装和 Plugin/Agent 加载矩阵通过，模型评测未运行 |
...
3. `dshVersions` 声明 release matrix 的精确验证目标，实际通过须有对应候选的运行证据；`dshRange` 声明支持范围。当前 `0.2.26` 的范围与矩阵目标都收敛到 rc.1，新增候选不继承历史版本的实际通过记录；没有 npm 发布制品的 `0.1.3-alpha.1` 不进入矩阵或 peer，即使其官方 tag 能构建且 Standard composition 摘要相同。
