...
| --- | --- | --- | --- |
| `0.2.31` | `0.2.31` | `0.1.5-rc.1` | 未发布候选；沿用 canonical 0.3.15 / runtime contract 6；新增原生子代理调用摘要、匹配职责的共享路由指引与父会话模型回执，修复合同重复注入、手工标签绕过证据门、并行回执覆盖及迟到回执误判当前轮；分开风险提醒、职责路由和职责只读权限，修复研究任务传递并收拢职责生命周期，补齐候选文档一致性校验 |
...
2. `0.2.15` 至 `0.2.18` 只支持精确的 `0.1.1-rc.2` 与 `0.1.2-alpha.2`；`0.2.19` 只支持精确的 rc.2 与 alpha.4。`0.2.20`、`0.2.21` 与 `0.2.22` 按已发布合同保留 rc.2 和后续 `0.1.2` prerelease 范围；`0.2.23` 至 `0.2.27` 只支持精确 `0.1.2-rc.1`。当前已发布的 `0.2.28` 与 `0.2.29` 仅面向精确 `0.1.5-rc.1`。
