...
3. `dshVersions` 只表示 release matrix 完整跑过的精确版本，`dshRange` 才是支持范围。当前 `0.2.25` 的范围与矩阵目标都收敛到 rc.1，新增候选不继承历史版本的实际通过记录；没有 npm 发布制品的 `0.1.3-alpha.1` 不进入矩阵或 peer，即使其官方 tag 能构建且 Standard composition 摘要相同。
