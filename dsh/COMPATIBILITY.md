...
| --- | --- | --- | --- |
| `0.2.32` | `0.2.32` | `0.1.5-rc.1` | 未发布候选；canonical 0.3.16 / runtime contract 6，修复原生调用卡片布局及缺失轮次标签，增加绑定托管 reviewer 的不可变证据分页；模型评测按实际冻结指纹另行记录 |
...
2. `0.2.15` 至 `0.2.18` 只支持精确的 `0.1.1-rc.2` 与 `0.1.2-alpha.2`；`0.2.19` 只支持精确的 rc.2 与 alpha.4。`0.2.20`、`0.2.21` 与 `0.2.22` 按已发布合同保留 rc.2 和后续 `0.1.2` prerelease 范围；`0.2.23` 至 `0.2.27` 只支持精确 `0.1.2-rc.1`。从 `0.2.28` 起（包括当前已发布的 `0.2.31` 及候选 `0.2.32`）仅面向精确 `0.1.5-rc.1`。
