...
test("model route failures distinguish invalid routes from environment and transient failures", () => {
  ...
  assert.equal(
    classifyModelRouteFailure({ code: "PI_AI_ERROR", message: "404: model gpt-x does not exist" }).kind,
    "deterministic",
  );
  assert.equal(classifyModelRouteFailure({ code: "AUTH", message: "401" }).kind, "environment");
  assert.equal(
    classifyModelRouteFailure({ code: "AUTH", message: "401: requested model does not exist for this account" }).kind,
    "environment",
  );
  assert.equal(
    classifyModelRouteFailure({ code: "PERMISSION", message: "model not found in permitted catalog" }).kind,
    "environment",
  );
  ...
  assert.deepEqual(
    classifyModelRouteFailure({
      ...
      failure: { code: "MODEL_NOT_FOUND", message: "nested missing model" },
    }),
  ...
  assert.deepEqual(
    classifyModelRouteFailure({
      code: "PI_AI_ERROR",
      message: "request failed",
      failure: { code: "AUTH", message: "nested credentials failed" },
    }),
...
test("model route probe uses non-generating call resolution and preserves exact route fields", async () => {
  ...
  await assert.rejects(
    requireModelRoute(
      async () => {
        const error = new Error("provider has no configured model");
