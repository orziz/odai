...
test("session ceiling directives require an explicit declarative session-scoped change", () => {
  for (const text of ["这个会话放开上限", "当前对话取消输出限制", "Remove the output cap for this session"]) {
...
test("controller output recovery requires an immediately preceding verified interruption and pure continuation", () => {
  const invalidInterruptions = [
    ...
    },
    {
      turn: 1,
      step: 1,
      budgetSource: "preexisting-request-ceiling",
      reason: "max-tokens",
      configuredMaxTokens: 500,
      effectiveMaxTokens: 500,
      outputTokens: 500,
  ...
  for (const data of invalidInterruptions) {
    const invalidEvents = [{ type: "odai/controller-output-interrupted", data }];
    prepareSessionOutputControl({
      ...
      append(type, appended) {
        invalidEvents.push({ type, data: appended });
      },
    });
    assert.equal(
      invalidEvents.some((event) => event.type === "odai/controller-output-recovery"),
      false,
    );
  }
  const events = [
  ...
  const append = (type, data) => events.push({ type, data: data });
  prepareSessionOutputControl({ events, text: "继续修复登录页", turn: 2, step: 1, userMessageId: "revised-2", append });
  assert.equal(
    events.some((event) => event.type === "odai/controller-output-recovery"),
    false,
  );
  prepareSessionOutputControl({ events, text: "继续", turn: 3, step: 1, userMessageId: "late-3", append });
  assert.equal(
    events.some((event) => event.type === "odai/controller-output-recovery"),
    false,
  );
  prepareSessionOutputControl({ events, text: "继续", turn: 2, step: 1, userMessageId: "continue-2", append });
  prepareSessionOutputControl({ events, text: "继续", turn: 2, step: 2, userMessageId: "continue-2", append });
...
test("output policy validates explicit user-owned values and renders bounded guidance", () => {
  ...
  assert.deepEqual(resolveOutputPolicy({ concise: false, maxTokens: 2_500 }), { concise: false, maxTokens: 2_500 });
...
test("in-place responsibility budgets report unconfigured, inherited, and explicit ceilings", () => {
  ...
  assert.deepEqual(
    ...
    {
      planner: { source: "controller-policy", maxTokens: 500, warning: "responsibility-inherits-controller-ceiling" },
...
test("output policy store is atomic, repairable, locked, and resettable", async () => {
  ...
  try {
    ...
    writeFileSync(
      configPath,
      `${JSON.stringify({ schemaVersion: 1, policy: { concise: false, maxTokens: 2_500 } })}\n`,
      "utf8",
    );
    assert.deepEqual(readOutputPolicyStore(configPath).policy, { concise: false, maxTokens: 2_500 });
    rmSync(configPath);
    assert.deepEqual(effectiveOutputPolicy(configPath), { policy: { concise: true }, source: "default" });
    ...
    const adjustedEconomy = await tool.execute({ action: "set", mode: "economy", maxTokens: 1_200 }, execution);
    ...
    assert.equal(
      readdirSync(resolve(root, "odai")).some((entry) => entry.startsWith("output.json.invalid-")),
      true,
    );
...
test("output policy store fails closed on unreadable state instead of repairing it", () => {
  ...
  try {
    ...
    assert.equal(
      readdirSync(root).some((entry) => entry.startsWith("output-as-directory.invalid-")),
      false,
    );
  } finally {
    rmSync(root, { recursive: true, force: true });
...
test("owned store locks reject live writers, reclaim dead owners, and preserve successors", () => {
  ...
  try {
    ...
    assert.throws(() => acquireOwnedStoreLock(configPath, "Odai output configuration"), /is being updated; retry/u);
    rmSync(lockPath);
