...
function fixture() {
  const agent = { session: { header: { cwd: "/work" }, snapshotEvents: () => [], append() {} } };
  let task = "task-one";
  let seq = 0;
  const notices = [];
  const options = {
    taskFor: () => task,
    onRepeated(_execution, notice) {
      notices.push(notice);
    },
  };
  ...
  const finish = (execution, value) => monitor.observe(execution, { isError: false, value });
  const run = (exitCode, command = "npm test") => {
...
test("background receipts are associated and deduplicated; running jobs do not imply failure", () => {
  const f = fixture();
  const receipt = (id, status, detail) =>
    f.finish(f.call("job_output", { job_id: id }), { job: { id, status, detail }, text: "" });
...
test("late receipts cannot supersede newer outcomes or tasks, and runtime copies share deduplication", () => {
  ...
  const first = f.call();
  f.monitor.start(first);
  twin.start(first);
  const failure = { isError: false, value: { exitCode: 1 } };
  f.monitor.observe(first, failure);
  twin.observe(first, failure);
  assert.equal(f.notices.length, 0);
  const old = f.call("bash", { command: "npm test", run_in_background: true });
  f.monitor.start(old);
  f.finish(old, { kind: "background", jobId: "old" });
  f.run(0);
  ...
  f.monitor.start(earlierTask);
  f.finish(earlierTask, { kind: "background", jobId: "earlier-task" });
  f.newTask();
