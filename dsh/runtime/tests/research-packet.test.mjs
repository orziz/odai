...
import { parseResearchPacket, renderResearchPacket, verifyResearchPacketSources } from "../build/research-packet.mjs";
...
test("research packet verifies source existence, boundaries, line numbers, and exact excerpts", (t) => {
  ...
  try {
    ...
    writeFileSync(
      resolve(root, "logs/incidents.md"),
    ...
    assert.throws(
      () =>
        verifyResearchPacketSources(
          parseResearchPacket(JSON.stringify(packet({ facts: [{ ...packet().facts[0], excerpt: "different" }] }))),
          root,
        ),
      /does not match/u,
    );
      ...
      () =>
        verifyResearchPacketSources(
          parseResearchPacket(
            JSON.stringify(
              packet({
                facts: [packet().facts[0], { ...packet().facts[1], source: { path: "logs/missing.md", line: 1 } }],
              }),
            ),
          ),
          root,
      ...
      () =>
        verifyResearchPacketSources(
          parseResearchPacket(
            JSON.stringify(
              packet({
                facts: [packet().facts[0], { ...packet().facts[1], source: { path: "logs/incidents.md", line: 99 } }],
              }),
            ),
          ),
          root,
    ...
    assert.throws(
      () =>
        verifyResearchPacketSources(
          parseResearchPacket(
            JSON.stringify(packet({ facts: [{ ...packet().facts[0], excerpt: "different" }, packet().facts[1]] })),
          ),
          root,
        ),
      /does not match/u,
        ...
        () =>
          verifyResearchPacketSources(
            parseResearchPacket(
              JSON.stringify(
                packet({
                  facts: [{ ...packet().facts[0], source: { path: "config/escaped.md", line: 1 } }, packet().facts[1]],
                }),
              ),
            ),
            root,
...
test("research packet rejects weak or unsafe provenance", () => {
  assert.throws(
    () =>
      parseResearchPacket(
        JSON.stringify(
          packet({ facts: [packet().facts[0], { ...packet().facts[1], source: { path: "../private.md", line: 1 } }] }),
        ),
      ),
  ...
  assert.throws(
    () =>
      parseResearchPacket(
        JSON.stringify(
          packet({
            facts: [packet().facts[0], { ...packet().facts[1], source: { path: "logs/incidents.md", line: 0 } }],
          }),
        ),
      ),
    /positive integer/u,
  ...
  assert.throws(
    () => parseResearchPacket(JSON.stringify({ ...packet(), recommendation: "retry three times" })),
    /unknown fields/u,
  );
