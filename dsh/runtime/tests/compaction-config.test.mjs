...
test("compaction target validates explicit model options and applies only to compaction", () => {
  assert.deepEqual(
    resolveCompactionTarget({ provider: " openai ", model: " gpt-5.6-luna ", reasoningEffort: " high " }),
  ...
  assert.throws(
    () => resolveCompactionTarget({ provider: "", model: "luna" }),
    /\.provider must be a non-empty string/u,
  );
  assert.throws(
    () => resolveCompactionTarget({ provider: "openai", model: "luna", reasoningEffort: " " }),
    /\.reasoningEffort must be a non-empty string/u,
  );
  assert.throws(
    () => resolveCompactionTarget({ provider: "openai", model: "luna", effort: "max" }),
    /unknown fields: effort/u,
  );
  ...
  assert.throws(
    () =>
      applyCompactionTarget(Object.freeze({ ...compaction, model: "gpt-5.6-sol" }), {
        provider: "openai",
        model: "gpt-5.6-luna",
      }),
    /immutable request/u,
...
test("all compaction routes receive one integrity protocol without touching ordinary calls", () => {
  ...
  const originalMessages = [
    { id: "stock", role: "user", content: [{ type: "text", text: "Stock compaction instruction" }] },
  ];
  ...
  assert.throws(() => applyCompactionStateProtocol({ purpose: "compaction" }, target), /mutable message envelope/u);
});
test("compaction model store is atomic, repairable, locked, resettable, and child-protected", async () => {
  ...
  try {
    ...
    assert.deepEqual(initial, { action: "show", configPath, source: "inherit", requiresNextCompaction: false });
    assert.throws(() => tool.execute({ action: "show", provider: "openai" }, execution), /must be omitted for show/u);
    assert.throws(
      () => tool.execute({ action: "show", reasoningEffort: "high" }, execution),
      /must be omitted for show/u,
    );
    assert.throws(
      () => tool.execute({ action: "set", provider: "openai" }, execution),
    ...
    assert.deepEqual(last(events), { action: "set", target: { provider: "openai", model: "gpt-5.6-luna" } });
    ...
    assert.equal(
      readdirSync(resolve(root, "odai")).some((name) => name.startsWith("compaction.json.invalid-")),
      true,
    );
    const removed = await tool.execute({ action: "remove" }, execution);
    assert.deepEqual(removed, { action: "remove", configPath, source: "inherit", requiresNextCompaction: true });
