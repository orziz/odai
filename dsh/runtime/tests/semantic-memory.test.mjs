...
function memoryEntry(entries, description = "memory entry") {
  return required(entries[0], description);
}
function agentWithEvents(events) {
  return { session: { header: {}, snapshotEvents: () => events, append() {} } };
}
function toolExecution(tool, agent) {
  return { name: tool.name, agent };
}
function captureOutcome(outcomes, description = "capture outcome") {
...
test("native claimed input previews the newest batch but requires matching commitment for permission", () => {
  ...
  assert.equal(
    currentDirectInput(agent, { minimum: "claimed", messages: [directMessage("latest", "改写内容")] }),
    undefined,
  );
...
test("native claim authority does not survive cancellation, rejection, replacement, or restart", () => {
  for (const invalidate of ["abort", "reject", "closed", "new-turn", "disposed", "session", "restart", "conflict"]) {
    ...
    if (invalidate === "conflict")
      events.push({ type: "user/message", seq: 3, data: directMessage("claim", "改写内容") });
...
function agentFor({ id, cwd, turn = 1, text, messageId = `${id}-message` }) {
  ...
  return { agent: { session: { header: { id, cwd }, snapshotEvents: () => events, append() {} } }, message, turn };
}
...
test("quoted durable wording is data while direct preferences retain their exact excerpt", () => {
  const report =
    "- **语义记忆的口径和治理不一致**：用户没说“记住”时，DSH 也会用正则自动抓“以后/默认/我们决定…”这类偏好";
  assert.deepEqual(discoverAutomaticMemoryCandidates(report), []);
  for (const [open, close] of [
    ...
    const quoted = `${open}以后默认使用 npm。以后统一使用 yarn${close}`;
    assert.deepEqual(discoverAutomaticMemoryCandidates(`检测到 ${quoted} 这段内容。`), []);
    const direct = "本项目以后统一使用 pnpm";
    assert.deepEqual(
      discoverAutomaticMemoryCandidates(`检测到 ${quoted}。${direct}。`).map((item) => item.value),
      [direct],
    );
    const preference = `以后默认使用 ${open}pnpm${close}`;
    assert.deepEqual(
      discoverAutomaticMemoryCandidates(`${preference}。`).map((item) => item.value),
      [preference],
    );
  }
  assert.equal(
    discoverAutomaticMemoryCandidates("I prefer tools that don't change the project's formatting.").length,
    1,
  );
});
test("quoted wording cannot widen memory scope, impose a constraint, or authorize correction", () => {
  for (const [open, close] of [
    ['"', '"'],
    ["“", "”"],
    ["`", "`"],
    ...
    const scope = memoryEntry(
      discoverAutomaticMemoryCandidates(`本项目以后默认使用${open}所有项目通用${close}的 lint 配置`),
    );
    assert.equal(scope.scope, "project");
    const category = memoryEntry(
      discoverAutomaticMemoryCandidates(`本项目以后默认使用 pnpm，别碰${open}必须保留的旧脚本${close}`),
    );
    assert.equal(category.category, "preference");
    const correction = memoryEntry(
      discoverAutomaticMemoryCandidates(`本项目以后默认使用 pnpm，保留 README 里${open}改为 yarn${close}那一节`),
    );
    assert.equal(correction.correction, false);
    assert.equal(correction.subject, "package-manager");
  }
  const direct = memoryEntry(discoverAutomaticMemoryCandidates('所有项目以后必须改为 pnpm，保留"旧配置"'));
  assert.equal(direct.scope, "global");
  assert.equal(direct.category, "constraint");
  assert.equal(direct.correction, true);
});
test("only the authenticated direct-human message in the current open turn is eligible", () => {
  ...
  assert.equal(
    latestDirectUserMessage(
      agentWithEvents([...valid.session.snapshotEvents(), { type: "turn/end", seq: 12, data: { turn: 3 } }]),
    ),
    undefined,
  );
  assert.equal(
    latestDirectUserMessage(
      agentWithEvents([...valid.session.snapshotEvents(), { type: "turn/start", seq: 12, data: { turn: 4 } }]),
    ),
    undefined,
  );
  ...
  assert.equal(
    latestDirectUserMessage(
      agentWithEvents([...valid.session.snapshotEvents(), { type: "user/message", seq: 12, data: malformed }]),
    ),
    undefined,
  );
  for (const source of [
  ...
  for (const malformedHuman of [
    ...
    assert.equal(
      latestDirectUserMessage(
        agentWithEvents([...valid.session.snapshotEvents(), { type: "user/message", seq: 12, data: malformedHuman }]),
      ),
      undefined,
      "a malformed newer human must not revive the older one",
    );
  }
  const newer = directMessage("newer", "改为使用 npm。");
  const updated = agentWithEvents([...valid.session.snapshotEvents(), { type: "user/message", seq: 12, data: newer }]);
  ...
  assert.equal(
    latestDirectUserMessage(valid, [{ ...human, content: [{ type: "text", text: "同 ID 的伪造内容。" }] }]),
    undefined,
  );
});
...
test("automatic capture deduplicates dual runtimes and retrieves only active scoped memory", () => {
  ...
  try {
    ...
    assert.equal(
      sameProject.some((entry) => entry.subject === "package-manager"),
      true,
    );
    assert.equal(
      sameProject.some((entry) => entry.scope === "global"),
      true,
    );
    const otherProject = retrieveSemanticMemories({ storePath, query: "调整 pnpm 和 UTC 时间格式", cwd: projectB });
    assert.equal(
      otherProject.some((entry) => entry.subject === "package-manager"),
      false,
    );
    assert.equal(
      otherProject.some((entry) => entry.scope === "global"),
      true,
    );
  ...
  try {
    ...
    for (const [index, text] of values.entries()) {
      ...
      const result = await tool.execute(
        { action: "consider", scope: "project", category: "preference", subject: "emotional-state", excerpt: text },
...
test("governance-suppressing preferences are rejected while neutral preferences remain eligible", async () => {
  ...
  try {
    ...
    for (const [index, text] of blocked.entries()) {
      ...
      const result = await tool.execute(
        { action: "consider", scope: "project", category: "preference", subject: "response-governance", excerpt: text },
        toolExecution(tool, fixture.agent),
      );
    ...
    const allowed = await tool.execute(
      { action: "consider", scope: "project", category: "preference", subject: "response-style", excerpt: allowedText },
...
test("legacy governance-suppressing records stay manageable but cannot be recalled or confirmed", async () => {
  ...
  try {
    ...
    writeFileSync(storePath, `${JSON.stringify(raw)}\n`);
    const confirming = agentFor({ id: "legacy-confirm", cwd, turn: 3, text: `确认这条候选记忆 ${legacy.id}` });
    ...
    const forgetting = agentFor({ id: "legacy-forget", cwd, turn: 4, text: `删除这条记忆 ${legacy.id}` });
...
test("a quoted correction cannot supersede an active memory", () => {
  const root = mkdtempSync(resolve(tmpdir(), "odai-memory-quoted-correction-"));
  try {
    const storePath = resolveMemoryStorePath(undefined, { DSH_HOME: root });
    const cwd = resolve(root, "project");
    mkdirSync(cwd);
    capture(storePath, agentFor({ id: "quoted-original", cwd, text: "本项目以后默认使用 pnpm。" }));
    capture(
      storePath,
      agentFor({ id: "quoted-new", cwd, text: '本项目以后默认使用 npm，保留 README 里"改为 yarn"那一节。' }),
    );
    const entries = readMemoryStore(storePath).entries;
    assert.equal(entries.length, 2);
    assert.equal(entries.find((entry) => entry.value.includes("pnpm"))?.status, "active");
    assert.equal(entries.find((entry) => !entry.value.includes("pnpm"))?.status, "pending");
    assert.equal(
      entries.some((entry) => entry.status === "superseded"),
      false,
    );
  } finally {
    rmSync(root, { recursive: true, force: true });
...
test("conflicting repeated candidates stay pending until explicit confirmation", async () => {
  ...
  try {
    ...
    const confirmed = await tool.execute(
      { action: "confirm", id: confirmedPendingId },
      toolExecution(tool, confirmation.agent),
    );
...
test("correction supersedes conflicts while forget and clear physically erase content", async () => {
  ...
  try {
    ...
    const pnpmEntry = required(
      afterCorrection.find((entry) => /pnpm/u.test(entry.value)),
      "superseded pnpm memory",
    );
    ...
    const forgetting = agentFor({ id: "session-forget", cwd, turn: 3, text: `删除这条记忆 ${npmEntry.id}` });
    ...
    const clearRequest = await tool.execute(
      { action: "clear", scope: "project" },
      toolExecution(tool, preflight.agent),
    );
    ...
    const authorized = agentFor({ id: "session-clear-authorized", cwd, turn: 5, text: authorizationPhrase });
    ...
    await assert.rejects(
      async () =>
        tool.execute(
          ...
          toolExecution(tool, correctionAgent.agent),
        ),
...
test("stale or closed-turn human text cannot authorize any memory mutation", async () => {
  ...
  try {
    ...
    const tool = createSemanticMemoryTool(storePath);
    const activeId = memoryEntry(readMemoryStore(storePath).entries).id;
    const pendingFixture = agentFor({
      id: "stale-pending",
      cwd,
      turn: 2,
      text: "请考虑把本项目的输出格式偏好记为 json",
    });
    const pending = await tool.execute(
      {
        action: "consider",
    ...
    const staleAgent = {
      session: { header: { id: "stale-replay", cwd }, snapshotEvents: () => staleEvents, append() {} },
    };
    const before = readFileSync(storePath, "utf8");
    for (const args of [
      {
        ...
        category: "preference",
        subject: "x",
        excerpt: "CLEAR ODAI GLOBAL MEMORY",
      },
      {
        action: "correct",
        scope: "project",
...
test("invalid memory is visible and only an exact global clear resets it", async () => {
  ...
  try {
    ...
    const projectRequested = await tool.execute(
      { action: "clear", scope: "project" },
      toolExecution(tool, preflight.agent),
    );
...
test("memory mode is user-controlled and every child action is denied", async () => {
  ...
  try {
    ...
    const disabled = await tool.execute(
      { action: "set-mode", mode: "off", excerpt: "关闭语义记忆" },
      toolExecution(tool, disabling.agent),
    );
    ...
    const enabled = await tool.execute(
      ...
      toolExecution(tool, enabling.agent),
    );
    ...
    const deploymentDisabled = await deploymentDisabledTool.execute(
      { action: "inspect" },
      toolExecution(deploymentDisabledTool, enabling.agent),
    );
    ...
    for (const args of [
      ...
      const rejected = await deploymentDisabledTool.execute(
        args,
        toolExecution(deploymentDisabledTool, enabling.agent),
      );
    ...
    for (const action of ["inspect", "search", "consider", "confirm", "correct", "forget", "clear", "set-mode"]) {
      assert.throws(
        () => tool.execute({ action }, toolExecution(tool, child)),
        /child agents may not inspect or change/u,
      );
    }
