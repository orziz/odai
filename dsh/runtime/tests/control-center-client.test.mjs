...
async function loadClientModule(packageId, entries) {
...
async function loadClient(packageId, entries) {
...
test("shared client projection separates proposal, same-turn, child, and handback evidence", async () => {
  ...
  const trace = client.projectTrace([
    ...
    {
      seq: 3,
      type: "odai/route-result",
      data: { turn: 1, step: 3, role: "reviewer", action: "delegate", status: "completed" },
    },
    { seq: 4, type: "odai/responsibility-returned", data: { turn: 1, step: 4, responsibility: "planner" } },
  ]);
  assert.deepEqual(
    Array.from(trace.items, (item) => item.state),
    ["proposal", "same-turn", "child", "handback"],
  );
...
test("late child receipts retain roles without hiding a sibling failure or changing current turn", async () => {
  ...
  const trace = client.projectTrace([
    {
      seq: 1,
      type: "odai/route-applied",
      data: { turn: 2, responsibility: "planner", routeMode: "child", status: "mismatch", childSessionId: "bad" },
    },
    {
      seq: 2,
      type: "odai/route-applied",
      data: { turn: 2, responsibility: "planner", routeMode: "child", status: "applied", childSessionId: "good" },
    },
    {
      seq: 3,
      type: "odai/route-applied",
      data: { turn: 1, responsibility: "reviewer", routeMode: "child", status: "applied", childSessionId: "late" },
    },
    {
      seq: 4,
      type: "odai/route-applied",
      data: { responsibility: "frontend", routeMode: "child", status: "unverified", childSessionId: "resident" },
    },
  ]);
  assert.equal(trace.currentTurn?.turn, 2);
...
test("timeline orders groups by recent evidence without defaulting to out-of-turn evidence", async () => {
  ...
  assert.deepEqual(
    Array.from(trace.turns, (turn) => turn.key),
    ["session", "turn-2", "turn-1"],
  );
  ...
  const sessionOnly = client.projectTrace([{ seq: 5, type: "odai/routing-configured", data: {} }]);
...
test("timeline helpers skip unchanged append-only evidence and bound mounted rows", async () => {
  ...
  assert.notEqual(
    client.traceFingerprint(events),
    client.traceFingerprint([...events, { seq: 3, time: 300, type: "odai/responsibility-returned" }]),
  );
  const items = Array.from({ length: 250 }, (_, index) => ({ key: `event-${index}` }));
  assert.deepEqual(
    Array.from(client.windowTurnItems(items, 100), (item) => item.key),
    items.slice(150).map((item) => item.key),
  );
...
test("trace data resolves through the uiConversation binding", async () => {
  ...
  const fallback = client.conversationTraceSource(conversation).getSnapshot();
...
function mountSurfaceHarness(client, prepare) {
  const effects = [];
  const live = new Set();
  ...
  const makePair = (label) => {
    ...
    return {
      ...
      binding() {
        return {
          target() {
            return { getSnapshot: () => ({ events: [] }), subscribe: () => () => {} };
          },
        };
      },
    };
...
test("Control Center activates through the rc.1 uiConversation service", async () => {
  ...
  assert.equal(
    harness.registrations.filter((entry) => entry.kind === "event")[0]?.value.kind,
    "odai-control-center:event",
  );
