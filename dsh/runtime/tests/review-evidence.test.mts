...
import type { DshAgent, DshEvent, UnknownRecord } from "../build/runtime-types.mjs";

const user = (id: string): DshEvent => ({ type: "user/message", data: { message: {
  id, role: "user", source: { kind: "user" }, content: [{ type: "text", text: "Review the refund transaction; preserve requirements and acceptance." }],
} } });
function capture(callId: string, seq: number, text: string): DshEvent[] {
  return [
    { type: "tool/call", seq, data: { callId, name: "bash", arguments: { command: "git diff -- src/database.js" } } },
    { type: "tool/result", seq: seq + 1, sourceEventSeqs: [seq], data: { message: {
      role: "user", source: { kind: "tool", callId }, content: [{ type: "tool-result", toolCallId: callId, isError: false, content: [{ type: "text", text }] }],
    } } },
  ];
}
const entry = (text: string, index = 1) => ({ index, source: "tool", label: "tool read", identity: `tool-call:${index}`, kinds: ["tool"], text });

test("snapshot recovers decisive content beyond both packet clips without disclosing another task or assistant claims", () => {
  const body = "diff --git a/src/database.js b/src/database.js\n" + " unchanged source line\n".repeat(500) + "+ledger and balance roll back together\n";
  const events = [user("old"), ...capture("old-secret", 1, "OTHER_TASK_SECRET"), user("current"),
    { type: "assistant/message", data: { content: [{ type: "text", text: "ASSISTANT_CLAIM_ONLY" }] } },
    ...capture("refund-diff", 10, body), capture("unlinked", 30, "FORGED_RESULT")[1]!,
    { type: "odai/tool-observed", data: { tool: "read", callId: "observed-only", isError: false, text: "OBSERVATION_ONLY" } },
  ];
  ...
  assert.notEqual(reviewEvidenceSnapshot(buildRoleContextPacket(agent, "reviewer", "Review implementation"))!.digest, snapshot.digest);
...
test("snapshot ids, digest, bounds, total budget and retention limits fail closed", () => {
  ...
  let exhausted = false;
  for (let n = 0; n < 40; n++) {
    try { read({ digest: snapshot.digest, action: "read", id: "tool-event-1" }); }
    catch (error) { assert.match(String(error), /budget exhausted/); exhausted = true; break; }
  }
  assert(exhausted);
  assert.doesNotThrow(() => snapshot.createReader()({ digest: snapshot.digest, action: "list" }), "budgets belong to a dispatch, not shared snapshots");
  ...
  assert.equal((capped.createReader()({ digest: capped.digest, action: "list" }).entries as UnknownRecord[])[0]!.id, "tool-event-2", "prefer the latest capture over earlier revisions");
  ...
  const withoutIdentity = createReviewEvidenceSnapshot([{ index: 1, source: "tool", label: "capture", kinds: ["tool"], text: "source" }]);
...
test("only the bound managed reviewer can retrieve a snapshot, and disposal revokes access", async () => {
  ...
  const parent: DshAgent = { session: { header: { id: "review-parent" }, snapshotEvents: () => [], append() {} } };
  const route = { provider: "openai", model: "review-model" };
  for (const role of ["reviewer", "planner"] as const) {
    let bound: DshAgent | undefined;
    const outcome = await runRoutedRole({ provider: "spawn", decision: { role }, roleContract: "immutable review", taskText: "review",
      agent: parent, signal: new AbortController().signal, roleRoute: route, reviewEvidence: snapshot,
      subagents: { async start(_provider, request) {
        const make = (parentSession = "review-parent"): DshAgent => ({ session: {
          header: { id: "child", parentSession, origin: "subagent" }, append() {}, snapshotEvents: () => [
        ...
        if (role === "reviewer") {
          assert.ok(reader);
          assert.equal(managedReviewEvidenceReader(make()), undefined, "copied labels cannot attach a second session");
          const result = await tool.execute({ digest: snapshot.digest, action: "read", id: "tool-event-1" }, { agent: bound, name: tool.name, callId: "page" });
          assert.equal(result.text, "decisive captured source");
          assert.deepEqual(activeOdaiToolNames(classifyContextActivation("review"), { child: true, reviewEvidence: true }), ["odai_review_evidence"]);
        } else assert.equal(reader, undefined);
        await assert.rejects(async () => tool.execute({ digest: snapshot.digest, action: "list" }, { agent: parent, name: tool.name, callId: "forged" }), /active managed reviewer/);
        return { localAgent: bound, result: Promise.resolve({ stopReason: "completed", output: [{ type: "text", text: "review returned" }] }), async dispose() {} };
      } },
    });
    assert.equal(outcome.status, "completed");
    assert.ok(bound);
    assert.equal(managedReviewEvidenceReader(bound), undefined);
    await assert.rejects(async () => tool.execute({ digest: snapshot.digest, action: "list" }, { agent: bound, name: tool.name, callId: "late" }), /active managed reviewer/);
  }
});
