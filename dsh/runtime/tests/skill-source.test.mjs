...
import { chooseSkillBundle, compareSkillVersions, loadSkillBundle } from "../build/skill-bundle.mjs";
import {
...
import { selectSharedSkillForTurn, sharedSkillSelection } from "../build/skill-selection-state.mjs";
...
function turnAgent(cwd, events = []) {
  return { phase: { turn: 1 }, session: { header: cwd ? { cwd } : {}, snapshotEvents: () => events, append() {} } };
}
function testExecution(origin) {
  return {
    name: "odai_skill_source_config",
    agent: { session: { header: origin ? { origin } : {}, snapshotEvents: () => [], append() {} } },
  };
}
function isRuntimeTool(value) {
...
function installBundle(root, version, marker = "") {
  ...
  if (marker) {
    writeFileSync(
      ...
      `${readFileSync(resolve(root, "SKILL.md"), "utf8").trimEnd()}\n\n${marker}\n`,
      "utf8",
    );
    writeFileSync(
...
test("bundle manifest validates complete content and full SemVer precedence", () => {
  ...
  for (const [name, relativePath] of Object.entries(bundled.manifest.referenceFiles)) {
    assert.equal(
      bundled.referenceContracts[name],
      readFileSync(resolve(canonicalRoot, relativePath), "utf8").trim(),
  ...
  try {
    const conflicting = loadSkillBundle(
      installBundle(resolve(scratch, "odai"), bundled.manifest.skillVersion, "CONFLICT"),
      { source: "user-dsh" },
    );
    ...
    writeFileSync(resolve(scratch, "odai", "SKILL.md"), "---\nname: not-odai\n---\n\nname: odai\n---\n", "utf8");
    assert.throws(() => loadSkillBundle(resolve(scratch, "odai", "SKILL.md")), /must declare name odai/u);
    ...
    const topologyManifest = JSON.parse(readFileSync(topologyManifestPath, "utf8"));
    ...
    const memoryPath = topologyManifest.referenceFiles.memory;
    delete topologyManifest.referenceFiles.memory;
    ...
    topologyManifest.referenceFiles.memory = topologyManifest.referenceFiles.dao;
    writeFileSync(topologyManifestPath, `${JSON.stringify(topologyManifest, null, 2)}\n`, "utf8");
    assert.throws(() => loadSkillBundle(resolve(topologyRoot, "SKILL.md")), TypeError);
    topologyManifest.referenceFiles.memory = memoryPath;
...
test("bundle files cannot escape lexical or realpath boundaries", (t) => {
  ...
  try {
    ...
    const lexicalManifest = JSON.parse(readFileSync(lexicalManifestPath, "utf8"));
...
test("auto source keeps project pins scoped and selects newer user installs elsewhere", async () => {
  ...
  try {
    ...
    const projectSelection = await resolveSkillSelection({ mode: "auto", bundled, cwd: resolve(projectA, "src"), env });
    ...
    const userSelection = await resolveSkillSelection({ mode: "auto", bundled, cwd: projectB, env });
    ...
    const forcedUser = await resolveSkillSelection({ mode: "user", bundled, cwd: projectA, env });
...
test("invalid and conflicting candidates continue to the next compatible source", async () => {
  ...
  try {
    ...
    assert.deepEqual(
      selection.rejections.map(({ source, reasonCode }) => [source, reasonCode]),
      [
  ...
  try {
    ...
    const skills = {
      async get(...arguments_) {
        ...
        return { name: "odai", source: "custom", provider: "fixture-custom", path: customPath };
      },
    };
...
test("source configuration is atomic, explicit, repairable, and denied to child agents", async () => {
  ...
  try {
    ...
    assert.throws(
      () => tool.execute({ action: "show", source: "auto" }, rootExecution),
      /source must be omitted for show/u,
    );
    ...
    assert.throws(() => tool.execute({ action: "set", source: "user" }, rootExecution), /is being updated; retry/u);
    rmSync(lockPath);
    assert.throws(
...
test("scoped runtime selection wins once and the global runtime follows it", async () => {
  ...
  try {
    ...
    const selected = await globalAssemble(assembly, context, () =>
      scopedAssemble(assembly, context, async () => assembly),
    );
...
test("runtime injects one project snapshot into both prompt and routed role contract", async () => {
  ...
  try {
    ...
    const ctx = fakeContext({
      subagents: {
        async start(_provider, request) {
          ...
          const events = [
            { type: "request/header", data: { header: { config: { provider: "fixture", model: "planner" } } } },
          ];
          return {
            localAgent: { session: { snapshotEvents: () => events } },
    ...
    createSessionEvidence({ root: resolveSessionEvidenceRoot(routingConfigPath) }).append(agent, gap.type, gap.data);
    ...
    assert.ok(
      prompt[0].text.includes(planning.contract.trim()),
      "planner receives its complete owner from the same selected snapshot",
    );
