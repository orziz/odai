...
function harness() {
  ...
  const agent = { currentText: "", session: { header: {}, snapshotEvents: () => [], append() {} } };
...
test("continuity requires explicit current-user persistence and exact user-authored values", async (t) => {
  ...
  assert.throws(
    () =>
      state.tool.execute(
        { action: "add", category: "care-preference", value: "我今天觉得心累。" },
  ...
  for (const refusal of [
    ...
    assert.throws(
      () => state.tool.execute({ action: "add", category: "care-preference", value }, execution(state.agent)),
      /explicit(?: affirmative)? request/u,
    );
    assert.equal(readHumanSafetyContinuityStore(state.storePath).entries.length, 0);
  }
  state.agent.currentText = "请不要保存";
  assert.throws(
    () => state.tool.execute({ action: "add", category: "care-preference", value: "不要" }, execution(state.agent)),
    /explicit(?: affirmative)? request/u,
  );
  ...
  assert.deepEqual(state.changes[0], { action: "add", category: "care-preference", entryId: entryId(added) });
  const quotedValue = "先确认我想要倾听还是建议";
  state.agent.currentText = `请跨会话保存这条照护偏好：“${quotedValue}”。`;
  const quoted = await state.tool.execute(
    { action: "add", category: "effective-support", value: quotedValue },
    execution(state.agent),
  );
  ...
  assert.throws(
    () =>
      state.tool.execute(
        { action: "add", category: "safety-plan", value: "联系一个可信任的人" },
        execution(state.agent),
      ),
    /byte-for-byte/u,
  );
...
test("continuity rejects ordinary memory content even when the requested category is safety-scoped", async (t) => {
  ...
  for (const [request, value] of [
    ...
    [
      "Please save: I own the project's crisis-recovery deployment and use pnpm.",
      "I own the project's crisis-recovery deployment and use pnpm.",
    ],
    [
      "Please save: When I am overwhelmed by code reviews, use pnpm for the project.",
      "When I am overwhelmed by code reviews, use pnpm for the project.",
    ],
  ]) {
    state.agent.currentText = request;
    assert.throws(
      () => state.tool.execute({ action: "add", category: "care-preference", value }, execution(state.agent)),
      /human-safety or care content/u,
    );
  }
  assert.equal(readHumanSafetyContinuityStore(state.storePath).entries.length, 0);
  ...
  const added = await state.tool.execute(
    { action: "add", category: "care-preference", value: careValue },
    execution(state.agent),
  );
  assert.equal(added.status, "added");
  for (const value of [
    ...
    const workRelatedCare = await state.tool.execute(
      { action: "add", category: "care-preference", value },
      execution(state.agent),
    );
  ...
  for (const [request, value] of [
    ...
    [
      "Please replace that entry with: Care preference: use npm for this project.",
      "Care preference: use npm for this project.",
    ],
    [
      "Please replace that entry with: I own the project's crisis-recovery deployment and use npm.",
      "I own the project's crisis-recovery deployment and use npm.",
    ],
    [
      "Please replace that entry with: When I am overwhelmed by code reviews, use npm for the project.",
      "When I am overwhelmed by code reviews, use npm for the project.",
    ],
  ]) {
    state.agent.currentText = request;
    assert.throws(
      () =>
        state.tool.execute(
          { action: "replace", entryId: entryId(added), category: "care-preference", value },
          execution(state.agent),
        ),
      /human-safety or care content/u,
    );
...
test("continuity is visible, correctable, exportable, and physically clearable across tool instances", async (t) => {
  ...
  const added = await state.tool.execute(
    { action: "add", category: "care-preference", value: original },
    execution(state.agent),
  );
  ...
  assert.throws(
    () =>
      nextSessionTool.execute(
        { action: "replace", entryId: entryId(added), category: "effective-support", value: replacement },
        execution(state.agent),
      ),
    /explicit affirmative request/u,
  );
...
test("continuity rejects child access and private contact data", async (t) => {
  ...
  assert.throws(
    () =>
      state.tool.execute(
        { action: "add", category: "safety-plan", value: "需要时联系 138-1234-5678" },
        execution(state.agent),
      ),
    /contact details/u,
  );
