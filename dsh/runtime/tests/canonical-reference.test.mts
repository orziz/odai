...
test("responsibilities receive their owner through the shared compiler and reject invalid content", () => {
  const contents = Object.fromEntries(Object.entries(bundle.orchestration.fileContents).map(([file, bytes]) => [file, Buffer.from(bytes, "base64").toString("utf8")]));
  const references = { planning: "PLANNING_OWNER", verification: "VERIFICATION_OWNER", craft: "CRAFT_OWNER" };
  const governance = { runtimeContract: bundle.manifest.runtimeContract, skillBody: "GOVERNANCE", referenceContracts: references };
  for (const [role, owner] of [["planner", "planning"], ["reviewer", "verification"], ["frontend", "craft"]] as const) {
    const contract = dshRoleContract(role, composeRoleContract(role, governance, bundle.orchestration.manifest, contents, { embedded: true }));
    ...
    for (const invalid of [undefined, "", 42]) {
      assert.throws(() => Reflect.apply(composeRoleContract, undefined, [role,
        { ...governance, referenceContracts: { ...references, [owner]: invalid } }, bundle.orchestration.manifest, contents]), /reference .* unavailable/u);
    }
  }
});
