...
function record(value) {
  assert.ok(isUnknownRecord(value));
  return value;
}
...
test("governance loads without orchestration files or a compiler dependency", () =>
  fixture(async (root) => {
    cpSync(canonical, resolve(root, "odai"), { recursive: true });
    for (const file of ["governance-bundle.mjs", "runtime-types.mjs"])
      cpSync(resolve(repo, "dsh/runtime/build", file), resolve(root, file));
...
test("composition passes captured payloads once and embeds only declared reference dependencies", () => {
  ...
  const contents = Object.fromEntries(
    Object.entries(bundled.orchestration.fileContents).map(([file, bytes]) => [
      file,
  ...
  const input = {
    runtimeContract: bundled.manifest.runtimeContract,
    skillBody: "GOVERNANCE_PAYLOAD",
    referenceContracts: bundled.governance.referenceContracts,
  };
  for (const role of Object.keys(manifest.roleFiles)) {
    ...
    assert.equal(
      composeRoleContract(role, input, manifest, contents, { embedded: true }).includes(input.skillBody),
      false,
    );
  }
});
test("invalid orchestration dependencies and empty resources fail without changing governance", () =>
  fixture((root) => {
    ...
    for (const mutate of [
      (value) => {
        record(record(value.rolePresets).planner).references = [];
      },
      (value) => {
        record(record(value.rolePresets).frontend).references = ["unknown"];
      },
      (value) => {
        value.delegationFile = "../outside.md";
      },
      (value) => {
        value.governanceContract = 99;
      },
    ]) {
      const value = record(JSON.parse(original));
      mutate(value);
    ...
    assert.throws(
      () => loadSkillBundle(resolve(canonical, "SKILL.md"), { orchestrationRoot: root }),
      /unavailable|empty/u,
    );
  }));
test("governance and orchestration identities change independently and snapshots remain immutable", () =>
  fixture((root) => {
    const core = resolve(root, "odai"),
      orchestration = resolve(root, "orchestration");
    cpSync(canonical, core, { recursive: true });
    cpSync(extension, orchestration, { recursive: true });
...
test("external scripts and neighboring orchestration are ignored; incompatible governance falls back without rewriting", () =>
  fixture(async (root) => {
    const project = resolve(root, "project"),
      external = resolve(project, ".dsh/skills/odai");
    ...
    const file = resolve(external, "manifest.json"),
      manifest = record(JSON.parse(readFileSync(file, "utf8")));
    manifest.skillVersion = "9.0.0";
    manifest.requiredFiles.push("scripts/compose-contracts.mjs");
    writeFileSync(file, JSON.stringify(manifest));
    const neighbor = resolve(external, "../odai-orchestration");
    mkdirSync(neighbor);
    ...
    for (const previous of [
      { schemaVersion: 5, runtimeContract: 8 },
      { schemaVersion: 3, runtimeContract: 7 },
    ]) {
...
test("copied agent preset and plugin resolve both bundles outside the repository", () =>
  fixture(async (root) => {
    for (const [name, packageSource] of [
      ["agent", "dsh/agent/preset/odai/package.json"],
      ["plugin", "dsh/plugin/package.json"],
    ]) {
