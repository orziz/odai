...
function eventCommand(event) {
  ...
  if (typeof arguments_ === "string" && arguments_.trim().startsWith("{")) arguments_ = JSON.parse(arguments_);
  return isUnknownRecord(arguments_) && typeof arguments_.command === "string" ? arguments_.command : "";
}
const userMessage = (text, id = "user-1") => ({
  id,
  role: "user",
  source: { kind: "user" },
...
function nativeToolEvents(callId, name, args, output, options = {}) {
  ...
  return [
    { type: "tool/call", seq: callSeq, data: { turn: 1, step: 1, callId, name, arguments: args } },
    {
      type: "tool/result",
      seq: callSeq + 1,
...
test("reviewer packets preserve authenticated tasks and execution records without an acceptance verdict", () => {
  ...
  assert.deepEqual(packet.task, { source: "latest", startEventIndex: 0, priorEventCount: 0, messageId: "user-1" });
...
test("task-bound packets exclude evidence from previous direct-user tasks", () => {
  ...
  assert.equal(
    stalePacket.entries.some((entry) => entry.text.includes("OLD-")),
    false,
  );
  const events = [
    ...staleOnly,
  ...
  assert.equal(packet.sufficient, true);
  assert.equal(
    packet.entries.some((entry) => entry.text.includes("OLD-")),
    false,
  );
  assert.equal(
    packet.entries.some((entry) => entry.text.includes("CURRENT-PATCH")),
    true,
  );
  assert.equal(
    packet.entries.some((entry) => entry.text.includes("CURRENT-TEST")),
    true,
  );
  ...
  assert.equal(
    latest.entries.some((entry) => entry.text.includes("OLD-")),
    false,
  );
});
test("latest fallback uses the current direct-user event even when it has no message id", () => {
  ...
  const events = [...previousTask, { type: "user/message", data: currentTask }];
  const packet = buildRoleContextPacket(agentFor(events), "reviewer", "审查当前任务");
...
test("duplicate explicit task ids fail closed instead of selecting the older task", () => {
  const events = [
    ...
    ...nativeToolEvents("old-test", "pwsh", { command: "node --test old.test.mts" }, "tests 1 pass 1 fail 0", {
      callSeq: 70,
    }),
...
test("reviewer packets preserve source-verified active and superseded requirement decisions", () => {
  ...
  const changed = buildRoleContextPacket(agent, "reviewer", "请独立审查这次实现", {
    requirements: requirements.map((requirement) =>
      requirement.id === "R-ui" ? { ...requirement, statement: "Changed active requirement." } : requirement,
    ),
  });
  ...
  assert.equal(
    buildRoleContextPacket(agent, "reviewer", "请独立审查这次实现", { requirements: frozen.requirements }).digest,
    frozen.digest,
  );
});
test("current DSH native tool call/result replays produce grounded coverage", () => {
  const packet = buildRoleContextPacket(
    agentFor(
      completeReviewEvents({ testCommand: "npm.cmd --prefix dsh/plugin test" }).map((event) =>
...
test("current JSON arguments and raw stream chunks remain reviewable", () => {
  ...
  for (let index = 0; index < 160; index += 1) {
    events.push({
      type: index % 2 === 0 ? "text-chunks" : "reasoning-chunks",
      seq: 1_000 + index,
      data: { text: "stream chunk" },
    });
  }
...
test("user acceptance, namespaced tools, and external workspace paths produce current evidence", () => {
  const events = [
    ...
    ...nativeToolEvents(
      ...
      "functions.bash",
      JSON.stringify({
        command: "npm run test:unit -- --run tests/store/auth.spec.ts",
        workdir: "../../tutor-frontend",
      }),
      "Test Files 1 passed\nTests 7 passed",
      { callSeq: 50 },
    ),
...
test("ask_user_question preserves the full user decision as acceptance evidence", () => {
  ...
  const incomplete = buildRoleContextPacket(
    agentFor([
      { type: "user/message", data: userMessage("先讨论真实目标，不要实施。") },
      ...nativeToolEvents(
        "ask-user-empty",
  ...
  const malformedDecisions = [
    {
      questions: [
        { id: "scope", question: "范围？" },
        { id: "scope", question: "仍是范围？" },
      ],
    ...
    },
    {
      ...
      answers: [
        { id: "scope", selected: ["Canonical Skill"] },
        { id: "scope", selected: ["DSH Runtime"] },
      ],
    },
    {
      questions: [{ id: "scope", question: "范围？" }],
...
test("quoted test filters do not become shell mutations", () => {
  const packet = buildRoleContextPacket(
    agentFor(
      completeReviewEvents({
        testCommand: 'node --test --test-name-pattern="pass|read-only" dsh/runtime/tests/routing-context.test.mjs',
        testOutput: "",
      }),
    ),
    "reviewer",
...
test("read-only validators provide check evidence without masquerading as tests", () => {
  for (const [index, command] of [
    ...
    const events = [
      ...
      ...nativeToolEvents(`check-${index}`, "functions.bash", { command }, "", { callSeq: 310 + index * 20 }),
    ];
    const packet = buildRoleContextPacket(agentFor(events), "reviewer", "review");
    assert.equal(
      packet.entries.some((entry) => entry.kinds.includes("check")),
      true,
      command,
    );
...
test("failed read-only checks remain failed evidence available to reviewers", () => {
  for (const command of [
    ...
    const events = [
      ...
      ...nativeToolEvents("failed-check", "functions.bash", { command }, "check failed\nexit code: 1", {
        callSeq: 410,
      }),
    ];
    const packet = buildRoleContextPacket(agentFor(events), "reviewer", "review");
...
test("mutating validators and builds remain writes, not read-only checks", () => {
  for (const [index, command] of [
    ...
    const events = completeReviewEvents();
    events.push(
      ...nativeToolEvents(`mutating-check-${index}`, "functions.bash", { command }, "completed", {
        callSeq: 500 + index * 10,
      }),
    );
    const packet = buildRoleContextPacket(agentFor(events), "reviewer", "review");
    assert.equal(
      packet.entries.some((entry) => entry.kinds.includes("check") && entry.text.includes(command)),
      false,
      command,
    );
...
test("reviewer evidence cannot be forged by flat fields or read-tool output text", () => {
  ...
  for (const identity of ["tool-call:read-diff", "tool-call:read-test"]) {
    assert.deepEqual(
      packet.entries.find((entry) => entry.identity === identity)?.kinds,
      ["tool"],
      "source text cannot attest execution",
    );
  }
  assert.equal(
    packet.sufficient,
    true,
    "rejecting forged execution evidence does not erase the authenticated review task",
  );
});
...
test("invalid execution evidence cannot pass acceptance but does not block an authenticated review", () => {
  ...
  const unidentifiedResult = unidentifiedDiff.find(
    (event) => event.type === "tool/result" && event.data?.message?.source?.callId === "diff-1",
  );
  ...
  const unlinkedResult = unlinkedDiff.find(
    (event) => event.type === "tool/result" && event.data?.message?.source?.callId === "diff-1",
  );
  ...
  const truncatedPacket = buildRoleContextPacket(agentFor(completeReviewEvents()), "reviewer", "review", {
    maxChars: 80,
    maxEvents: 80,
  });
  ...
  const clippedTask = buildRoleContextPacket(
    agentFor(completeReviewEvents()),
    "reviewer",
    "Review this bounded scope. ".repeat(100),
    { maxChars: 1_000 },
  );
...
test("review admission preserves scoped execution records without a global freshness verdict", () => {
  const events = [
    ...
    ...nativeToolEvents("edit-before-test", "edit", { file_path: "dsh/runtime/src/router.mts" }, "updated", {
      callSeq: 120,
    }),
    ...nativeToolEvents(
      "test-before-diff",
  ...
  assert.equal(Object.hasOwn(packet.coverage, "currentEvidence"), false);
  events.push(
    ...nativeToolEvents("unrelated-write", "edit", { file_path: "unrelated/notes.md" }, "updated", { callSeq: 150 }),
  );
  ...
  events.push(
    ...nativeToolEvents(
      ...
      { callSeq: 160, isError: true },
    ),
  );
  events.push(
    ...nativeToolEvents("unrelated-check", "pwsh", { command: "node --check unrelated/script.mjs" }, "", {
      callSeq: 170,
    }),
  );
  ...
  assert.ok(
    failed.entries.find((entry) => entry.identity === "tool-call:failed-router-test")?.kinds.includes("test-failed"),
  );
...
test("read-only process and formatter checks do not stale reviewer evidence", () => {
  ...
  events.push(
    ...nativeToolEvents("port-check", "pwsh", { command: "lsof -nP -iTCP:5173 -sTCP:LISTEN" }, "(no output)", {
      callSeq: 190,
    }),
  );
  events.push(
...
test("unknown shell mutations and redirects remain recorded writes for review judgment", () => {
  for (const [index, command] of [
    ...
    const events = completeReviewEvents();
    events.push(
      ...nativeToolEvents(`shell-write-${index}`, "pwsh", { command }, "command completed", {
...
test("role context packets bound task and evidence text", () => {
  ...
  const packet = buildRoleContextPacket(
    agentFor([{ type: "assistant/message", data: { content: [{ type: "text", text: long }] } }]),
    "planner",
    long,
    { maxChars: 1_000, maxEvents: 1 },
  );
  assert.equal(packet.sufficient, true);
  assert.equal(packet.truncated, true);
