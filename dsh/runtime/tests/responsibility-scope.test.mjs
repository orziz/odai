...
test("scope owner keeps transition order and rejects stale scope mutations", () => {
  const events = [];
  const agent = { session: { header: {}, snapshotEvents: () => events, append() {} } };
  ...
  const options = { turn: 1, startStep: 2, role: "planner", route: roleRoute };
  ...
  assert.deepEqual(
    ...
    [
      ...
      "odai/responsibility-scope-claimed",
  ...
  assert.deepEqual(
    events.map((event) => event.data.scopeId),
    [active.id, active.id, active.id, active.id, next.id],
  );
  assert.equal(events[3].data.stopStep, 3);
  assert.equal(owner.stop(agent, "route-mismatch", { scopeId: active.id }), undefined);
  assert.throws(
    () => owner.claim(agent, pending, { step: 2, baseRoute, temporaryRoute: roleRoute, routeMode: "same-turn" }),
    /stale/,
  );
...
test("coexisting owners preserve live scopes and recover disposed owners", () => {
  for (const dispose of [false, true]) {
    ...
    const active = first.claim(agent, pending, {
      step: 1,
      baseRoute,
    ...
    assert.throws(
      () => second.claim(agent, active, { step: 2, baseRoute, temporaryRoute: roleRoute, routeMode: "same-turn" }),
      /stale/,
    );
    if (dispose) {
      first.dispose();
...
function pendingScope(role = "planner") {
...
test("all in-place responsibilities share terminal, direct-user, and turn ownership boundaries", () => {
  for (const role of ["planner", "reviewer", "frontend"]) {
    ...
    assert.equal(
      responsibilityScopeStopReason(scope, {
    ...
    assert.equal(responsibilityScopeStopReason(scope, { type: "turn/end", data: { turn: 1 } }), "turn-ended", role);
  }
});
test("scope continues only for tool chains and stops at ownership boundaries", () => {
  ...
  assert.equal(responsibilityScopeStopReason(scope, { type: "turn/end", data: { turn: 1 } }), "turn-ended");
});
test("durable scope evidence recovers the base route once and never revives a stopped scope", () => {
  const events = [
    { type: "odai/responsibility-scope-started", data: { scopeId: "scope-1", role: "reviewer" } },
    {
      ...
      data: { scopeId: "scope-1", role: "reviewer", baseRoute, temporaryRoute: roleRoute },
    },
  ];
  assert.equal(latestDanglingResponsibilityScope(events)?.scopeId, "scope-1");
  events.push({
    type: "odai/responsibility-scope-stopped",
    data: { scopeId: "scope-1", role: "reviewer", baseRoute, temporaryRoute: roleRoute, reason: "terminal-response" },
  });
