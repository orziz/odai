...
import type { PromptAssembly } from "../build/runtime-types.mjs";

// Pinned rc.2 host wording, including cross-tool defaults and the real authority contract.
...
const todoHead = "Send the ENTIRE list every call. Use it to plan multi-step work and show progress: add one todo per concrete step before you start. ";
const todoTail = "Mark a todo `completed` the moment it is done (do not batch completions), and allow no `in_progress` item only once all work is complete. Statuses: pending, in_progress, completed.";
const ralph = "Use the ralph tool ONLY when the direct human explicitly asks for a Ralph loop or fresh-agent iterative execution. Use same-session goal tools for ordinary long-running objectives, and plain subagents or workflows for bounded delegation and fan-out.";
...
test("optional work tools agree without weakening actual schemas, authority or todo concurrency", () => {
  for (const active of [
    ...
    const original: PromptAssembly = {
      sections: [{ name: "tool:goal", text: goal }, { name: "sandbox", text: "Require permission for protected writes." }, { name: "tool:ralph", text: ralph }],
      tools: [
        ...
        { name: "ralph", description: "Use only when the direct human explicitly asks for Ralph or fresh-agent iteration. Ordinary long-running same-session work belongs to goal tools." },
      ],
    };
    ...
    assert.doesNotMatch(result.tools[1]!.description!, /add one todo per concrete step|do not batch completions|while work remains/);
