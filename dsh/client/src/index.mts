...
window.__ModuleLoader__.load({
  ...
  factory: (require_) => {
    ...
    async function callEvidence(connection: any, sessionId: string, signal?: AbortSignal): Promise<unknown[]> {
      const response = await connection.rpc.call("/odai-control-center", "evidence", { sessionId }, signal);
      ...
      return Array.isArray(response.value.events) ? response.value.events : [];
    }
    ...
    function Timeline({ trace, selectedKey, onSelect }: { trace: ProjectedTrace; selectedKey?: string; onSelect(key: string): void }) {
      return h("section", { className: "odaiCC__timeline" },
        ...
        trace.turns.length ? h("div", { className: "odaiCC__turns" }, trace.turns.map((turn) => h("details", {
          key: turn.key,
          open: turn.key === trace.currentTurn?.key,
          className: "odaiCC__turn",
        },
          h("summary", null, h("strong", null, turn.title), h("span", null, `${turn.items.length} 个事件`)),
          h("div", { className: "odaiCC__events" }, turn.items.map((item) => h(EventRow, {
            key: item.key,
            item,
    ...
    function Inspector({ item }: { item?: TraceItem }) {
      return h("aside", { className: "odaiCC__inspector" },
        ...
        item ? h(React.Fragment, null,
          ...
          h("pre", { className: "odaiCC__raw" }, JSON.stringify(item.raw, null, 2)),
    ...
    function ControlCenter({ useSession, connection, onClose }: { useSession: any; connection: any; onClose(): void }) {
      ...
      useEffect(() => {
        ...
        const refreshEvidence = async () => {
          try {
            const events = await callEvidence(connection, sessionId, controller.signal);
            if (active) {
              setStoredEvidence(events);
              setEvidenceError("");
            }
          } catch (cause) {
...
`;
    ...
    exports.__testing = { projectTrace, stateOf, roleOf, shouldOwnSurface };
