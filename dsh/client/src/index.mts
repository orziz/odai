  ...
  factory: (require_) => {
    ...
    function projectTrace(events: readonly unknown[] = []): ProjectedTrace {
      ...
      const groups: TraceGroup[] = [...turns.values()].map((group) => Object.freeze({
        ...group,
        title: group.turn === undefined ? "未标注轮次的会话事件" : `第 ${group.turn} 轮`,
      ...
      return h("section", { className: "odaiCC__flow" },
        ...
        h("div", { className: "odaiCC__sectionHead" },
          h("div", null,
            ...
            h("p", { className: "odaiCC__nativeNote" }, "显示当前已载入会话的调用记录。收到工具回执不代表子任务完成或职责模型已核实。"),
          ),
          h("span", { className: "odaiCC__eventCount" }, `${calls.length} 次调用回执`),
        ),
        calls.length ? h("div", { className: "odaiCC__nativeCalls" }, calls.slice(-5).map((item) => {
          const data = record(item.raw);
          return h("button", { key: item.key, type: "button", className: "odaiCC__flowRole odaiCC__nativeCall", onClick: () => onSelect(item.key) },
            h("span", { className: "odaiCC__roleNode" },
        ...
        calls.length > 5 ? h("p", { className: "odaiCC__nativeNote" }, `共 ${calls.length} 次，此处显示最近 5 次；其余可在事件时间线查看。`) : null,
      );
    }
