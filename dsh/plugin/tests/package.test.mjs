...
test("bundle patch resolves the packaged runtime through the package export", async () => {
  ...
  assert.equal(
    metadata.files.some((entry) => entry.includes("tests") || entry.includes("scripts")),
    false,
  );
});
test("CLI clearly retires the incompatible legacy repair command", () => {
  ...
  const runtimeLocated = spawnSync(process.execPath, [bin, "legacy-session-repair", "--dsh-home", pluginRoot], {
    encoding: "utf8",
  });
  ...
  const missingHome = spawnSync(process.execPath, [bin, "legacy-session-repair", "--dsh-home", "--yes"], {
    encoding: "utf8",
  });
...
test("DSH process spawn supports Windows npm command shims", () => {
  ...
  spawnDsh("dsh", ["--profile", "web"], { cwd: pluginRoot }, { platform: "win32", execute });
  spawnDsh("dsh", ["--profile", "web"], { cwd: pluginRoot }, { platform: "linux", execute });
  spawnDsh("C:\\tools\\dsh.cmd", ["--profile", "web"], { cwd: pluginRoot }, { platform: "win32", execute });
  assert.deepEqual(calls, [
    { command: "dsh.cmd", args: ["--profile", "web"], options: { cwd: pluginRoot, shell: true } },
    { command: "dsh", args: ["--profile", "web"], options: { cwd: pluginRoot } },
    { command: "C:\\tools\\dsh.cmd", args: ["--profile", "web"], options: { cwd: pluginRoot, shell: true } },
  ]);
});
test("Windows plugin probe terminates the DSH process tree", () => {
  ...
  const child = {
    ...
    exitCode: null,
    kill() {
      throw new Error("taskkill should be preferred");
    },
  };
  terminateDsh(child, {
    platform: "win32",
  ...
  assert.deepEqual(calls, [{ command: "taskkill", args: ["/pid", "42", "/T", "/F"], options: { stdio: "ignore" } }]);
});
