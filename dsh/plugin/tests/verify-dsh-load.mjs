...
const wrapper = [
  ...
  "  });",
  "  if (ctx.tools.get('odai_skill_evolution')) throw new Error('retired odai_skill_evolution is still registered');",
  ...
  "  });",
  "  Promise.all([guardsReady, configProbe, sourceProbe, outputProbe, compactionProbe, memoryProbe, safetyProbe, continuityProbe, reviewProbe]).then(() => {",
...
try {
  await Promise.all([writeFile(patchPath, patch, "utf8"), writeFile(wrapperPath, wrapper, "utf8")]);
  const child = spawnDsh(dsh, ["--profile", "headless", "--patch", patchPath, "odai load probe"], {
    cwd: repoRoot,
    env: { ...process.env, DSH_HOME: resolve(scratch, "home"), DSH_TELEMETRY_DISABLED: "1" },
