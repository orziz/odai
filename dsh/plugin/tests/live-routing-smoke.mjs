...
const pluginPath = [resolve(pluginRoot, "runtime/index.mjs"), resolve(pluginRoot, "../runtime/build/index.mjs")].find(
...
try {
  ...
  if (args.controllerMaxTokens !== undefined) {
    ...
    await writeFile(
      resolve(outputRoot, "output.json"),
      `${JSON.stringify(
        { schemaVersion: 1, policy: { concise: true, maxTokens: args.controllerMaxTokens } },
        null,
        2,
      )}\n`,
      "utf8",
  ...
  const run = await runProcess(dsh, ["--profile", "headless", "--patch", patchPath, args.task], {
    cwd: workspaceRoot,
  ...
  const report = {
    ...
    requested: {
      ...
      planner:
        args.plannerProvider === undefined
          ? null
          : {
      ...
      frontend:
        args.frontendProvider === undefined
          ? null
          : {
...
function parseArgs(argv) {
  ...
  if (!argv.includes("--task") && (parsed.mode === "auto" || parsed.mode === "execute") && parsed.plannerModel) {
    parsed.task =
      "这是 planner 路由的集成验证，不修改文件。请独立比较支付请求的超时与重试方案：当前缺口是因果假设尚未验证，而延迟、重复交易和回退成本之间存在路线取舍；预期贡献是区分可证实条件并给出决策方法，不拍具体参数。请用 odai_responsibility_gap 记录这一 planner 缺口，以本条任务为证据来源，按配置派发并核对回执；不要声称已有运行数据或实现验收。";
  }
  ...
  if (
    ["auto", "execute"].includes(parsed.mode) &&
...
function renderPatch(input) {
  ...
  const role =
    input.frontendProvider !== undefined
      ? {
          name: "frontend",
  ...
  const routing =
    input.mode === "default"
      ? []
      : ["        routing:", `          mode: ${input.mode}`, "          provider: spawn", ...roleMapping];
  return [
...
function verifySmoke(sessions, options) {
  ...
  const expectedRoute = ["auto", "execute"].includes(options.mode)
    ? targetRole === "frontend"
      ? { provider: options.frontendProvider, model: options.frontendModel, reasoningEffort: options.frontendReasoning }
      : { provider: options.plannerProvider, model: options.plannerModel, reasoningEffort: options.plannerReasoning }
    : {
  ...
  const controllerRoute = controllers.flatMap((session) => session.requestRoutes).find(sameRoute);
  ...
  if (expectedMode === "off") {
    ...
    if (!controllerRoute) {
      ...
      if (controllerRoute.maxTokens !== expectedMaxTokens) {
        errors.push(
          `frontend request/header maxTokens was ${controllerRoute.maxTokens}, expected ${expectedMaxTokens}`,
        );
      }
      if (
        options.frontendMaxTokens !== undefined &&
      ...
      errors.push(
        `planner in-place route maxTokens was ${controllerRoute.maxTokens}, expected ${options.controllerMaxTokens}`,
      );
    }
  } else {
    ...
    if (
      !children.some((session) => session.requestRoutes.some((route) => sameRoute(route) && route.maxTokens === 2_048))
    ) {
...
async function readSessions(root, evidenceRoot) {
  ...
  for (const file of files.filter((candidate) => candidate.endsWith("session.jsonl"))) {
    ...
    sessions.push({ header, events: [...records.slice(1).map(parseEvent), ...(id ? (evidence.get(id) ?? []) : [])] });
  }
  return sessions;
}
...
function summarizeSession(session) {
  ...
  for (const event of session.events) {
    ...
    if (event.type === "assistant/chunk" && event.data?.chunk?.type === "usage" && isRecord(event.data.chunk.usage)) {
