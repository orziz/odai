...
The unpublished `0.2.21` candidate accepts formal `@deepseek-ai/dsh@0.1.1-rc.2` plus `>=0.1.2-alpha.5 <0.1.2`; rc.2, alpha.5, and rc.1 are the exact real-load matrix anchors, and rc.1 is the source composition. Its dual client activation fixes the published `0.2.20` Control Center, which can block alpha.5/rc.1 Web boot while waiting for rc.2-only conversation services. npm registry publication remains a separate follow-up. Published `0.2.19` retains its exact rc.2 + alpha.4 contract, and `0.2.15` through `0.2.18` retain their historical alpha.2 contract. Later in-range prereleases are accepted under the upstream SemVer contract without being described as already tested; alpha.4, other `0.1.1` previews, and stable `0.1.2` remain outside this range.
