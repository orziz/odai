...
Published `0.2.21` and the current `0.2.22` release accept formal `@deepseek-ai/dsh@0.1.1-rc.2` plus `>=0.1.2-alpha.5 <0.1.2`. The current real-load matrix covers the distinct rc.2 and rc.1 contracts, with rc.1 as the source composition; alpha.5 remains inside the range but receives no redundant standalone check because its relevant composition and client contract are identical to rc.1. The dual client activation introduced in `0.2.21` fixes the published `0.2.20` Control Center, which can block alpha.5/rc.1 Web boot while waiting for rc.2-only conversation services. Published `0.2.19` retains its exact rc.2 + alpha.4 contract, and `0.2.15` through `0.2.18` retain their historical alpha.2 contract. Later in-range prereleases are accepted under the upstream SemVer contract without being described as already tested; alpha.4, other `0.1.1` previews, and stable `0.1.2` remain outside this range.
