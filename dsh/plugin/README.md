...
- task-state responsibility gaps, authenticated task-bound schema 3 packets that exclude prior-task session evidence and fail closed on unresolved bindings, inline same-model planner reuse, controller-owned implementation after planner handback, complete current reviewer evidence with controller-local incomplete-packet fallback, formal model-route preflight, and buffered compaction fallback without partial-output contamination;
...
Older releases wrote `odai/*` records into DSH's core log. DSH 0.1.5-rc.1 refuses unknown historical v0 events even when marked `ignorable: true`; adding that flag no longer restores readability. The old `legacy-session-repair` entry is retired and returns a clear error without opening or modifying session logs, including when passed `--yes`.

Normal install, update, uninstall, and runtime paths do not scan, merge, or rewrite those records. Current SDK support covers the integration and supported native logs; it does not claim that historical v0 Odai logs have been migrated. Existing data and backups remain user-owned.
...
The `0.2.39` source candidate bundles governance `0.7.1` (schema `5`, runtime contract `9`) and `odai-orchestration` `0.2.0` (schema `1`), the same skills as published `0.2.38`. Published `0.2.37` bundles governance `0.7.0` with the same schemas, runtime contract, and orchestration version. The governance manifest has no roles or orchestration dependency. DSH combines selected governance with its own captured orchestration, preserving separate digests and one immutable composition for prompts, references and role contracts. The packaged compiler never executes external bundle scripts or discovers a neighboring orchestration. Incompatible older sources fall back visibly without rewriting their files; an incompatible explicit `skillPath` fails fast.

## Retired skill evolution

Releases before `0.2.37` could refine both governance and orchestration Markdown in `$DSH_HOME/odai/skill-evolution`. Published `0.2.37` retired that overlay together with `odai_skill_evolution`, `ODAI_DISABLE_EVOLUTION`, and `governance.evolutionRoot`; later releases, including the `0.2.39` candidate, preserve that retirement. Previously active refinements stop taking effect after upgrading and restarting DSH. The runtime no longer reads the store, no package install, update, repair, or removal deletes it, and a leftover `evolutionRoot` is ignored.
...
A running DSH process keeps the bundled bytes loaded at startup, so update Plugin files only while DSH is stopped and restart to expose the new upstream.
...
The default smoke inherits `agent-default-model`, omits the routing block, and checks that a high-impact question alone does not invent a responsibility gap: one controller, zero children, the original `request/header`, and no missing-planner, upgrade, result, or whole-turn protection event. `observe` checks the same separation; `off` emits no routing events. Explicit planner `auto` and `execute` require `--planner-provider` plus `--planner-model`; their default test task asks the controller to record a concrete planner gap before routing. `--task` can supply another task with that gap. Planner `auto` requires a same-turn upgrade and `execute` requires one verified child. Frontend `auto` requires `--frontend-provider` plus `--frontend-model`, zero children, and matching request-header and budget-override evidence. The script has no built-in model name or reasoning effort. Live smoke calls models and requires explicit `--yes`; offline tests do not claim that result.
...
The current `0.2.39` source candidate uses an open DSH npm peer (`*`) so future releases and prereleases are not rejected by version alone. The tested baseline is `0.1.7-rc.1`; newer hosts are not automatically certified and must retain the required runtime services. Published `0.2.38` has the same peer, but DSH `0.1.7` rejects its Odai notices, memory packets and compaction instructions and fails the turn; use `0.2.39`. In contrast, published `0.2.37` targets only `@deepseek-ai/dsh@0.1.5-rc.2`; historical `0.2.32` retains its exact rc.1 contract. Control Center declares both `connection` and `webServer` for RPC registration, while governance remains available without the Web transport. The shared host supports Plugin-only and Agent installations. Published releases retain their historical compatibility entries; historical v0 Odai log migration is outside the current claim, as explained above.
