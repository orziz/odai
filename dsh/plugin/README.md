...
This candidate reads independent governance schema `5` / runtime contract `8` and bundles `odai-orchestration` `0.1.0` (schema `1`). The governance manifest has no roles or orchestration dependency. DSH combines selected governance with its own captured orchestration, preserving separate digests and one immutable composition for prompts, references and role contracts. The packaged compiler never executes external bundle scripts or discovers a neighboring orchestration. Incompatible older sources fall back visibly without rewriting their files; an incompatible explicit `skillPath` fails fast. Older active evolution is not automatically migrated.
...
`odai_skill_evolution` accepts exact replacements only in declared governance and orchestration Markdown. Governance paths include `SKILL.md`, `assets/task-state.md`, and `references/*.md`; orchestration paths use the `orchestration/` prefix, including its entry, references, roles and delegation contract. It rejects manifest edits, undeclared files, scripts, runtime JavaScript, symlinks, devices, path escapes, stale file hashes, and untracked changes. Each generation stores immutable base and result bundles plus content-addressed provenance; changing its bundle, metadata, or lineage makes validation fail closed.
...
`propose` and `rebase` create inactive candidates. `validate` replays their patches and returns an activation phrase bound to that exact generation. Any governance or orchestration `SKILL.md`, governance `references/dao.md`, or orchestration delegation-contract change, or any replacement that does not preserve its old text, is classified `breaking` and requires `ACTIVATE BREAKING ODAI EVOLUTION <generation-id>` instead of the standard `ACTIVATE ODAI EVOLUTION <generation-id>`. This conservative file-level rule includes frontmatter, preamble, every core section, and added conflicting instructions without coupling evolution to the canonical release validator's current wording.
