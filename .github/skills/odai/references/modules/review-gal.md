...
8. 仅当运行环境被明确识别为 Copilot，且满足 `references/dao/parallel-consensus-playbook.md` 的完整能力条件时，才允许并行开启完整 gal 审查；本轮并行任务组至少包含两种不同模型，可包含 1 个与主流程同模型的子 agent。内部收束、冻结结论后的独立复查与分歧升级统一按该手册执行；其他环境或能力不完整时，一律按原单流程完成，不得虚构多模型参与。
