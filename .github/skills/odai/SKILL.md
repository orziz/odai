...
9. 完整能力支持时才启用多 agent 多模型合议：仅当运行环境被明确识别为 Copilot，且满足 `references/dao/parallel-consensus-playbook.md` 的完整能力条件时，才允许在内部启用多 agent 多模型合议；本轮所有参与的子 agent 都必须显式承接同一份 `odai` skill 约束，至少要拿到同版 canonical 入口、当前命中模块与必要 support files 的规则，不得让外部通用 agent 脱离本 skill 自行发挥；是否可启用、如何触发、怎样收束与何时升级统一按该手册执行。若当前环境无法把统一 skill 约束可靠传递到子 agent，则视为能力不足，回到各模块原流程，不得虚构多模型参与。
