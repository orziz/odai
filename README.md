...
如果你只记默认入口：开发推进先用 `harness-dev`；方向、边界、主路与先手裁决先用 `harness-dao`。
...
2. 不知道先用哪个时：想持续推进开发任务，用 `harness-dev`；想先定方向、边界、主路和先手，用 `harness-dao`。
...
`harness-dao` 更适合“先定势，再落手”：先收目标、边界、主路、胜点、庙算和先手，再决定是否进入实施。
它偏方向与路线裁决，强调先定道、再收术、后落法。
...
- “用 `harness-dao` 接这个任务：先定道，再给主路、胜点、庙算和先手，输出尽量短。”
