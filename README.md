...
The current `0.3.11` candidate has no new model score or measured token savings. The frozen `0.3.10` snapshot scored 4/4 on the targeted C19 capability handoff with GPT-5.6 Sol/high; that result does not certify the current candidate. See [`docs/evaluation-results.md`](docs/evaluation-results.md) for the complete output, scope, preservation checks, and fingerprints. The frozen `0.3.9` result remains 18/19 and 142/144, including its historical C19 score of 3/4. That result and the older scores below do not certify current sources.
