...
## Unreleased — DSH 0.2.31 / canonical 0.3.15

- 对齐能力选择与风险治理：未经核实的高影响改动先给总控证据提醒，不自动制造 planner 缺口、索要模型配置或锁住整轮；引用示例与纯评估不会触发该自动提示。真实职责缺口仍按用户配置路由，缺失或失败职责所依赖的高影响操作保持保护，未知工具副作用不放行。宿主尚不能把插件侧路径核验绑定到实际写入，因此不宣称支持安全的文件级局部放行。
