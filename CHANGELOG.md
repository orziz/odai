...
- registry 已确认双包 `0.2.30` 发布，后续修复归入新候选 `0.2.31`；canonical 更新为 `0.3.15`，runtime contract `6`、required files 与精确 DSH `0.1.5-rc.1` peer 不变。不迁移或改写历史成绩，也不宣称新模型能力计分已通过；本地合同、回归与有界语义复核不冒充跨模型能力或成本证明。

## 2026-09-11 — DSH 0.2.30

- npm registry 确认 Plugin 与 Agent 分别于 `2026-09-11T09:48:41.114Z`、`2026-09-11T09:48:47.106Z` 发布，未提供 `gitHead`。该版本不再作为未发布候选；上方记录描述当前源码，不将后续修复追认为已发布制品的内容。
