...
- 双包同步使用新候选 `0.2.27`，canonical 升为 `0.3.10`，runtime contract `6`、required files 与精确 rc.1 peer 不变。已发布的 `0.2.26` / `0.3.9` 不复用来承载不同内容。早期 `0.3.9` 冻结快照的 Sol/high 为 **18/19 pass、142/144**，C19 未通过；这些分数不证明当前候选，详见 `docs/evaluation-results.md`。当前尚无新模型评测，不宣称质量、token、费用或延迟增益。
