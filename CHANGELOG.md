...
- 编排技能升为 `0.2.0` 并跟进 runtime contract `9`：安装、更新与卸载说明移入 `references/install.md`，不再随通用宿主 controller 进入每轮上下文；Claude 角色文件不再同时预加载 odai 技能与内嵌治理正文。
...
- 清理 DSH 的空 craft / responsibility-gap 提示段、未使用的角色加载包装函数与 controller 合同组合；总控仍直接加载治理，通用宿主 controller 生成能力保留。移除 `odai-dsh-plugin/governance` 导出的未使用 `DEFAULT_CHILD_DENIED_TOOLS`：直接导入该旧常量的外部代码需移除依赖；实际子代理权限继续由 allow-list 与执行守卫决定。
- 修复自动语义记忆把引号或行内代码中的“以后默认／我们决定”等示例误存为用户决定的问题；引用内容也不再扩大作用域、改变类别或授权更正替代旧记录。明确偏好可继续包含带引号的值，保留原文来源。此修复只影响新提取，不自动改写既有记忆。
...
- 官方 npm registry 确认 Plugin 于 `2026-09-22T07:55:34.060Z`、Agent 于 `2026-09-22T07:57:08.176Z` 发布，两包当前 `latest` 均为 `0.2.36`。下载制品的 SHA-512 与 registry integrity 一致；制品包含治理 `0.6.1` / schema `5` / runtime contract `8`、编排 `0.1.0` / schema `1`，peer 均为精确 `0.1.5-rc.2`。两包的治理与编排文件逐字节匹配仓库 `6c6b411` 对应目录；registry 未提供 `gitHead`，不据此推断完整发布提交。
- 整理完整执行主线，以可核查事实支撑判断和行动，区分事实、推断、假设与预期效果；默认采用最简充分做法并复用有效判断、产物和证据。规划确定整体路线与连接，制作承接方案，验证合并重复判断并按属性收口；交接材料按受托问题取用，日常关怀不先要求选择风格，交付先给结果、按需展开。保留完整要求、关键沟通、授权、事实、人身安全和真实完成边界。
- 在 Odai 的 DSH 提示适配中移除宿主默认推断持久 goal、从普通“继续”自动恢复 goal，以及多步骤任务必须先建 todo 的要求。自动跨轮续作改为用户明确要求时使用；工具接口、执行授权和已有 goal 状态管理保持。
- 审查入口与验收结论分开：真实任务可在缺少 diff、测试未运行或失败时开始审查；托管 reviewer 可在受托范围内使用 `read/glob/grep` 补读源码，执行回执仍由不可变快照提供。删除统一证据新鲜度裁决，保留写入、成功、失败与顺序，由具体属性决定证据是否仍适用；补读源码不证明测试执行。
- 缺失映射、路由失败和观察模式不再触发总控整轮只读；真实活动中的只读职责、宿主权限、用户授权与独立性要求保持。历史 sidecar 中的路由失败记录仍保留，但职责释放后重载不会重新启用其写入保护；planner 缺口码未退役。删除未被执行链使用的授权词句分类器，编排正文改为按需读取。
- 修正演进提案测试对正文中连字符数量的隐含依赖，使用显式重复与缺失匹配夹具；精确唯一匹配与授权守卫保持，被拒绝的替换不生成候选或改变激活状态。修正失败证据测试的类型收窄。恢复“不以少提问或多确认为优劣”和“未检查不能写成无需改动”。
- 修复纯只读检查的 `&&` 链未计入验证证据的问题；失败仍记录为失败，混合或写入命令不冒充只读检查。
- 单轮 Web 评测支持显式采集未回答的用户问题，保留完整选项与原生事件并核对回执；不代答、不伪造完成、不将失败或超时判为成功。C04 按事实依据、补证路径与决策质量评分，必要提问不因等待回答扣分。
- 行为和耗时改善不能由正文重写或程序检查通过推定，证据与限制见 `docs/evaluation-results.md`。

## DSH 0.2.35 / canonical 0.6.0

- 按 [`version-policy.json`](version-policy.json) 的新版本禁用数字 `4` 规则，从已发布 `0.2.33` 跳至 `0.2.35`；`0.2.34` 是策略跳过项，不是本轮部分发布消耗版本的记录。
- 细化兼听辨据、受托权限、可撤回行动与纠错边界；机制按用户结果与负担衡量，不按角色、规则或检查数量衡量。
- 合并总控内部重复以及 planner、reviewer 已从公共合同收到的内容；任务包来源规则归入一个生产端入口，保留统一路由与职责自身限制。
- 删除 Markdown 逐句、关键词、词序及语义反转测试，移除篇幅审定与告警；校验器只检查元数据、文件和引用，合同加载由运行时代码测试覆盖。问题是文字匹配约束写法，却不能证明模型遵守治理；预期减少合法改写的同步维护与无关检查。必须保持授权、人身安全、真实完成边界，以及加载、快照、演进与权限保护。相关程序验证只覆盖这些可执行机制，不证明正文的模型效果不退化；没有历史误报或真实回退拦截率统计，当前正文的定向行为观测也没有前后对照，不能宣称退役净收益已经测得。篇幅是成本观测，不是质量或放行标准；其审阅门一并退役，不据篇幅下降宣称行为改善。
- 治理独立为 canonical `0.6.0` / manifest schema `5` / runtime contract `8`，精神内核保留原句。角色、委派、路由脚本与合同组合移入可安装的 `odai-orchestration` `0.1.0` / schema `1`；Hooks 适配器归入 `integrations/hooks/`。
- DSH 双包内置治理和编排，分别记录内容身份并组合为同一回合快照；独立治理加载器不导入编排。演进保存两份清单，保护两份入口和受托边界。旧 contract 来源或演进明示降级且保留原数据，外部脚本不执行。
- 收窄按优化动机强制测试、默认外显规格和固定计划标签，按实际影响保留验证；共通语言、用户决定与证据复用规则归回治理，受托合同只补交接责任。通用 reviewer 允许宿主授权的版本绑定只读取证，DSH 的快照与工具限制继续由私有适配和权限执行。researcher 按材料规模、专业性与压缩价值判断分工，DSH 接受单来源、单条有效事实，保留逐字引用验证、路径边界与容量上限，避免为数量门槛扩大调查。
- 规划事实基线明确区分方案所需能力与项目已有能力，未知依赖保留核实及缺失时的处理条件；受托方案评测允许有依据的专业选择与可调整初值，不以缺少唯一实现为由强制交还用户。
- 保持精确 DSH rc.2 兼容；结构与代码行为检查不作为模型效果或协作收益证明。

## 2026-09-20 — DSH 0.2.33 / canonical 0.5.0

- 官方 npm registry 确认 Plugin 与 Agent 分别于 `2026-09-20T08:25:51.082Z`、`2026-09-20T08:27:25.570Z` 发布，版本标识已消耗；后续改动归入 `0.2.35`。

- canonical 采用 manifest schema `3` / runtime contract `7`：`SKILL.md` 保持完整技能入口与核心规则的唯一正文；共享组合器按职责投影核心，并组合独立受托边界、专业合同与必要 reference owner。五个命名职责保留为声明式预设，不成为固定阶段或能力上限。
- Codex、Claude、Copilot 生成器与 DSH 消费同一可信组合器；子职责不再继承总控调度和最终交付流程。DSH 只叠加自身执行机制，工具权限与模型映射不由 canonical 预设授予。复制安装后的 Agent preset 自带静态解析边界；所选外部 bundle 的脚本始终作为数据，不动态执行。
- 总控保有完整目标与最终验收责任；planner 区分局部判断与正式计划，reviewer 区分局部属性与明确委托的完整需求覆盖。finding 绑定既有要求、安全边界及证据，偏好不产生否决权；局部回交不能冒充整体完成，总控也不得忽略真实失败。
- 有界交接保留相关用户原文、必要依赖与可访问原件；托管子职责绑定父线程合同快照，通用 child 也加载受托边界。后续只补变更和失效证据，不因截断循环重派或重复成功检查。
- 演进快照、digest 与授权分类同步保存模块和预设拓扑，核心与受托边界的追加修改仍需受保护授权。旧 schema/contract 的外部 bundle 或失效的 active evolution 明示降级，保留原数据；不承诺自动迁移或无损恢复旧演进。
- 双包、安装版本门和 release matrix 收敛到精确 `DSH 0.1.5-rc.2`；历史 `0.2.32` 及更早版本的 rc.1 记录保留，不新增跨版本兼容分支。
- 职责执行策略直接读取真实 scope 的权限合同，与模型映射和 dispatch 分开；同一策略用于原生工具目录和提示过滤，并与执行守卫共用权限来源。只读职责不再展示写入、shell、委派及未知副作用工具，回交后恢复总控能力；宿主无法安装目录限制时仍保留提示过滤与执行保护。
- 按 rc.2 的真实顺序，在 pre-step 职责切换后刷新已捕获的工具与提示数组；保持宿主浅复制后的数组引用，修复首次职责请求及首次总控恢复仍使用旧工具列表的问题。权限未变化时不重复组装，也不额外调用模型。
- 同线程 reviewer 派发后消费原审查请求，不再因缺少仅独立 child 才需要的证据包而回交后重复启动；独立审查缺包时的等待与证据门保持。
- 子代理不再注入只适用于总控的输出策略；完整职责 owner、证据包、父子身份校验和实际请求预算保持。rc.2 尚未证明任务级写域隔离，子代理继续只读。当前不宣称模型质量、token、延迟或费用增益。

## 2026-09-18 — DSH 0.2.32 / canonical 0.3.16

- 官方 npm registry 确认 Plugin 与 Agent 分别于 `2026-09-18T02:15:30.285Z`、`2026-09-18T02:34:26.315Z` 发布，peer 均为精确 `@deepseek-ai/dsh@0.1.5-rc.1`。版本标识已消耗，后续改动归入 `0.2.33`。

- 修复托管 reviewer 无法补读被裁剪证据的问题：保留当前任务已捕获的不可变工具证据，在真实父子会话绑定下按需分页；快照、响应和调用次数均有上限，结束即撤销访问，禁止工作区扫描、执行与跨任务读取。摘要预算和用户模型选择不变；上游未捕获内容仍保留为未判定。
- canonical `0.3.16` 仅为通用审查合同增加宿主可选的只读证据分页例外，保持无该能力时的默认行为；required files、runtime contract `6` 与精确 DSH `0.1.5-rc.1` peer 不变。历史模型成绩不迁移；以截断反例、原生记录回放与隔离宿主检查验证本次系统修复。

- 完善维护用 DSH canary 适配器：认证原生游标与固定边界分页、request/claimed step/turn 归属校验、同会话多轮与重启续作、隔离只读裁判和完整中间证据；兼容代际 session 日志与独立 system message，失败清理前保留原生记录，不将失败改记为成功。
- 澄清局部保护能力边界：宿主 write/edit intent 已能携带实际目标和 execution actor；现有同步工具保护尚未接入该机制，intent 的单槽组合及其他副作用仍需验证，不新增未经证明的文件级放行。

- 修复控制中心原生调用卡片误占职责连线列的问题，改为自适应卡片布局；将缺少轮次字段的事件准确标为“未标注轮次的会话事件”，保留回执数量、最近五次展示和任务完成/模型核实的证据边界。

## 2026-09-14 — DSH 0.2.31 / canonical 0.3.15

- npm registry 确认 Plugin 与 Agent 分别于 `2026-09-14T10:12:42.670Z`、`2026-09-14T10:12:54.527Z` 发布。该版本标识已消耗；后续改动归入 `0.2.32`。

- 对齐能力选择与风险治理：未经核实的高影响改动先给总控证据提醒，不自动制造 planner 缺口、索要模型配置或锁住整轮；引用示例与纯评估不会触发该自动提示。真实职责缺口仍按用户配置路由，缺失或失败职责所依赖的高影响操作保持保护，未知工具副作用不放行。该版同步工具保护尚未接入宿主的实际目标写入链，因此不宣称支持安全的文件级局部放行。
- 研究任务在 child 与 same-turn 中保留真实 gap、预期贡献、证据线索和用户原任务，消除固定支付/因果调查模板对其他问题的改写。职责 scope 的启动、认领、停止、恢复和 owner 绑定收拢到现有单一 owner；同线程研究、规划、审查的只读权限独立于风险保护状态。
- 修复 Agent/Plugin 并存时另一个实例误停活动职责的问题：同进程共享活动 scope 的真实 owner，回交、请求失败、保护释放由原 owner 完成；卸载后仍可通过持久证据恢复总控。保护记录按 scope 与来源区分，避免同一步骤的职责保护释放吞掉随后建立的风险保护；已有 sidecar 记录 ID 按原样保留。
- 双包版本校验同时核对唯一 Unreleased owner、canonical 标识与兼容表首行/倒序，防止当前候选被重复记作历史或误归旧版本；保留 registry 与 tarball 摘要的发布事实门，本地文档一致性不冒充未发布证明。真实模型 smoke 按风险直做与显式职责缺口分开验收。
- 补齐原生委派与命名职责的衔接：总控在各工具呈现模式下都能取得匹配职责使用共享配置与托管证据门的指引。手工 planner/frontend 子代理取得对应合同，托管子代理不重复注入已传正文；researcher/reviewer 必须通过托管证据校验，运行时身份绑定直接父会话和唯一子会话，复制标签不能绕过。
- 本地子代理模型回执按明确父子关系回到控制中心并保留职责、实际模型、失败状态和独立子会话身份；并行回执不碰撞，迟到回执不切换当前轮，兄弟子代理成功不掩盖另一项失败。后续常驻子代理工作没有已核实父轮次关联时留在会话级；路由核实不冒充任务完成，不迁移旧日志。远程缺少实际模型回执仍不认定路由成功。
- 控制中心在当前轮命名职责图之外展示已载入会话证据中的原生子代理调用回执，去重并保留调用报错和未知轮次；不把工具回执当作子任务完成或已配置职责模型的执行证明，不改变路由配置或权限。
- 沿用已发布 `0.2.30` 的 canonical `0.3.15`、runtime contract `6` 与精确 DSH `0.1.5-rc.1` peer；本版不重新归属前版的技能整理，也不新增模型能力或成本成绩。

## 2026-09-11 — DSH 0.2.30 / canonical 0.3.15

- 补齐重试、评估交付、状态询问与叫停、诊断副作用、最终回执及安装许可交付的文本合同保护，分别验证关键分句删除和等义措辞。入口体积区分审视目标与固定已审基线，只对新增增长提醒，仍保留完整数值且不自动上调基线。
- 入口合并可检验判断与证据纠偏，将交互授权规则归入行动边界；支撑选择明确为“以完整结果、判断质量和可靠性为前提的最低充分支撑”。保留状态询问与叫停、真实副作用、最终回执及安装交付的具体约束，不以低于入口软阈值为删减能力的理由。
- 退役“总控包办全部制作”和固定角色即能力上限的通用限制；总控保留目标、整合、验证与最终交付责任。DSH 子代理保持只读，在明确委派制作时回交待应用补丁，专门研究、规划、审查合同和高影响执行门不放开；子代理提示不会重复累积或进入总控作用域。
- 未决取舍允许提出有依据的首选建议，但不能当成用户已确认的方案或启动依赖该决定的实施；高影响局部阻断不扩成其余已授权工作一并停手。支撑先满足完整结果和可靠性，不强制先试已知不足的轻方案，也不为缩小补丁机械删除合法替代路线。
- DSH 为 planner/reviewer 补入同一技能快照的 planning/verification 正文，沿用 frontend 的 craft 传递方式，修复职责引用了 owner 却无法取得正文的缺口。技能源加载测试验证正文与源文件一致；静态校验退役 references 的固定章节标题要求，仍保护核心合同与权限边界，不把文本检查冒充模型能力证明。
- canonical 明确讨论与确认也是共同澄清目标、比较可能性和校准理解的方式，不再将交流限制为缺信息或必须审批；须由用户决定的取舍仍保留。当前判断加入可检验解释、判别证据、完整依赖与反例反馈，简单问题仍直接完成。做法未知不自动判为能力不足，已有真实专长或独立性缺口时及时借力，不为自主而反复硬试。
- canonical 补齐整体整理的覆盖边界：局部任务不扩围，整体维护也不以周边修复代替本体交付。原生子代理与职责路由作为同一缺口的可选路径，复用有效回交，不为角色标签重复派发。
- planner/reviewer 通用合同退役无人解析的模式首行与路由卡片，回交保留完整目标、用户原文、未决取舍、验收和停止条件；planner 不自行实施或最终交付，reviewer 仍只读证据包。跨宿主 researcher 描述对齐“调用前有收益依据、回交后验贡献”。
- 可选 Hooks 在执行声明检查前解析实际 cwd，拒绝经符号链接离开项目，保留项目内别名；Hooks 验证临时目录退出即清理。
- 移除无现役消费者的旧日志改写实现及其专用历史测试，保留 CLI 退役提示、真实 SDK 拒绝/不写入 probe，并把现役 sidecar evidence 回归归位到独立测试。真实制品核验复用已有 finally 清理入口，失败也移除打包副本。

- Agent Control Center 把 bundle patch 纳入完整性检查，缺失时报告漂移并允许显式修复；独立安装入口在调用包管理器前核对精确支持的 DSH 版本，status 与卸载保留恢复用途。版本识别和排序复用 `semver`，支持合法 prerelease/build metadata，同时保持精确安装版本要求与禁止静默降级。
- 双包 `check` 统一委托根目录 `check:dsh`，移除重复构建与 Unix `find/xargs` 依赖，严格类型检查后另验渲染后的浏览器脚本。维护文档对齐同版同步发布、按需 craft、自有 preset、旧日志入口退役与生成产物清理。
- canonical 更新为 `0.3.15`，runtime contract `6`、required files 与精确 DSH `0.1.5-rc.1` peer 不变。不迁移或改写历史成绩，也不宣称新模型能力计分已通过；本地合同、回归与有界语义复核不冒充跨模型能力或成本证明。
- npm registry 确认 Plugin 与 Agent 分别于 `2026-09-11T09:48:41.114Z`、`2026-09-11T09:48:47.106Z` 发布，未提供 `gitHead`。两包已发布 tgz 的 28 个 canonical 文件均与当前 `0.3.15` 源码逐字节一致；制品尚无原生子代理调用摘要及后续职责回执衔接，这些增量归入 `0.2.31`。

## 2026-09-11 — DSH 0.2.29 / canonical 0.3.11

- 技能收紧无依据重复执行与完成声明：评估请求直接交付评估，明确叫停不借旧目标续作，依赖外部或异步结果的交付不以本地成功冒充完成。必要调查与重验保留，不以 token 上限替代质量判断。
- DSH 在既有工具入口观察同一命令的重复非成功结果，并发送去重的插件提醒；非零退出可能是预期红测或实验，不据此拦截重试或要求用户批准。记录按命令隔离，仅对应命令的成功结果清除其连续失败计数，无关读取、修改或成功命令不清空记录。结构化后台回执去重，旧任务或更早启动的迟到结果不覆盖新结果。同一任务内为受跟踪命令最多提醒一次，仅保留有界内存状态，不新增模型调用、持久账本或全局任务暂停。
- 官方 registry 已发布双包 `0.2.29`，Plugin 与 Agent 时间分别为 `2026-09-11T01:30:10.845Z`、`2026-09-11T01:30:19.978Z`，两个记录均未提供 `gitHead`；已发布 tgz 的 manifest 均确认为 canonical `0.3.11`、runtime contract `6`，peer 精确为 `@deepseek-ai/dsh@0.1.5-rc.1`。双包 `0.2.28` 与 canonical `0.3.10` 按历史保留；本版不继承历史模型成绩，也不宣称已测得 token、费用或延迟节省。

## 已发布 — DSH 0.2.28 / canonical 0.3.10

- 当前候选收敛到精确 `dsh@0.1.5-rc.1`，退役 `0.1.2-rc.1` 的当前支持；已发布版本的兼容记录保留。Agent 自行维护 preset 和能力合同，Standard 仅作上游参考，不再要求逐字跟随，也不自动引入它的新增工具。
- 原生真人消息不再被后台任务、技能目录等框架消息遮蔽。原生 inbox claim 只支持首轮提示、检索和本次请求配置；记忆写入、持续配置与责任授权等待内容一致的 `user/message` 提交，模型出站前再核对。拒绝、取消、改写及同轮新输入不能复活旧授权。常驻责任协议退役，必要用法归入工具说明，既有来源验证与子代理边界保留。
- 子代理创建时即限制继承工具，让原生技能目录在 pre-step 阶段看到正确可用性；首轮工具展示及原生工具说明与只读边界对齐，总控能力和其他宿主说明保留。
- 适配新版 persona 的 prefix/suffix 与 Control Center Web 传输依赖；非 Web 治理不依赖该传输。路由保护由工具执行前持久化的 `assistant/message` 生效，不要求已退役的 `assistant/chunk` 事件。
- 旧 `legacy-session-repair` 补标记入口退役，即使传入 `--yes` 也不打开或改写日志。新 SDK 仍拒绝含未知 Odai 事件的历史 v0 日志，当前不声明这些会话已迁移。
- canonical `0.3.10`、runtime contract `6` 不变；历史 C19 的 4/4 不冒充本次完整输入链或新版 SDK 的验收。

## 2026-09-09 — DSH 0.2.27 / canonical 0.3.10

- 官方 registry 已发布双包 `0.2.27`，Plugin 与 Agent 时间分别为 `2026-09-09T10:24:11.469Z`、`2026-09-09T10:24:27.900Z`。该版本标识已消耗，后续改动不复用。

- 普通任务直接完成：canonical 入口、制作、验证、计划、任务状态与总控合同明确额外工作必须解决当前具体问题，不默认先补测试、脚本、审计或需求台账。CSS、文案和局部布局使用足以判断结果的检查；新增测试须有实际回归价值。正式计划和状态载体按需使用，完整目标与未冲突要求不会被当前切片、状态询问或局部修订取代。
- 精神内核与 `事｜实｜法｜成｜界` 的定义保持不变，进一步明确五项是隐式判断而非填表流程。沿用已获授权的普通实施自主推进，以及未决用户取舍、扩围、外部后果、参考只读和高影响保护；已知环境缺失与预期红测仍不自动触发支撑升级。
- DSH 不再因实施授权自动注入完整 craft reference，也不因前端任务的规模、页面数或验收词汇自动切换责任；真实 frontend 缺口仍可取得已配置能力，制作资料继续按需可读。协作按具体贡献选择，首次调用不要求已有历史实测，失败只在有新修正依据时重试。简洁输出由结果领头，证据与下一步按相关性补充，不形成固定栏目或额外工作。
- schema 3 责任包绑定已认证用户任务，排除旧任务材料并拒绝无法唯一解析的绑定。既有 sidecar 中的责任回交按原生事件位置接入上下文，独立 scope 不因同一步而互相覆盖；回交不能制造用户验收或工具验证。diff 与成功检查都须晚于最后写入，但只读查看 diff 不再使先前成功检查失效，不要求为了顺序再跑一次。
- 压缩完整性说明覆盖 configured、inherit 和 fallback，保留完整目标、未完成要求、精确续作值与未确认动作结果，双 runtime 仍只添加一次。取消后的责任不重新启动，也不接受迟到成功输出；压缩的 error/aborted 结束不因缺少错误详情被误记为成功；aborted 即使没有详情或附有可重试错误码也不触发模型回退。
- 授权前交付补齐权威来源、安装或启用、后续操作及验证命令、影响与未知前提；引入外部能力时读取已有能力使用规则，诊断不绕过联网或安装授权。当前 canonical 的 Sol/high C19 已达到 4/4，确认完整命令链和内部 registry 访问前提，保持项按各自指纹记录。
- canonical 提示只渲染正文，完整源文件及 metadata 仍参与 bundle 身份。Agent 通过 `odai-governance.mjs` 稳定入口保持 Standard 全部能力；宿主 workflow/Ralph、子 agent 与 goal 的能力和原始参数契约保留，已撤回关键词隐藏与参数改写兼容层。
- 双包同步使用新候选 `0.2.27`，canonical 升为 `0.3.10`，runtime contract `6`、required files 与精确 rc.1 peer 不变。已发布的 `0.2.26` / `0.3.9` 不复用来承载不同内容。早期 `0.3.9` 冻结快照的 Sol/high 为 **18/19 pass、142/144**，C19 未通过；这些分数不证明当前候选，详见 `docs/evaluation-results.md`。当前仅有定向模型证据，尚无新全量或配对 A/B，不宣称跨模型质量、token、费用或延迟增益。

## 2026-09-07 — DSH 0.2.26

- 官方 registry 已发布 `odai-dsh-plugin@0.2.26` 与 `odai-dsh-agent@0.2.26`，时间分别为 `2026-09-07T02:32:33.576Z` 与 `2026-09-07T02:32:45.694Z`；两个 registry 记录均未提供 `gitHead`。已发布 tgz 的 manifest 确认为 canonical `0.3.9`、runtime contract `6`，peer 精确为 `@deepseek-ai/dsh@0.1.2-rc.1`。后续源码改动归入新的未发布候选。
...
## 2026-09-05 — DSH 0.2.23 rc.1 contract and Control Center

- `odai-dsh-agent` 与 `odai-dsh-plugin` 已发布 `0.2.23`；Plugin / Agent 的 registry 发布时间分别为 `2026-09-04T18:35:19.518Z` / `2026-09-04T18:35:50.972Z`，`gitHead` 均为 `b12a45058c528b5c1ea6cd56a570707d66ac2d1f`。peer 精确收敛到 `@deepseek-ai/dsh@0.1.2-rc.1`，退役 rc.2、alpha.5 范围兼容、旧 Agent renderer、旧 Web RPC 与 conversation service 分支，并只读取 rc.1 的 immutable Session snapshot。已发布版本的历史兼容记录不变；canonical skill 保持 `0.3.8`，runtime contract 保持 `6`，冻结的 provider-neutral CLI 不变。
- Plugin-only Web profile 不再假定 `connection` 已在 Odai runtime apply 前发布：Control Center runtime 订阅 Cordis `internal/service` 生命周期，在服务到达后注册一次 RPC，并由 effect disposer 保持共存去重与卸载。浏览器端只依赖 rc.1 `uiConversation` binding，长会话保持当前轮、时间线与 Inspector 独立滚动，闭合轮次不挂载事件行，展开轮次按 100 条逐批加载，unchanged evidence 不重复传输或渲染。Control Center host 缺失提示明确说明 host 已随 Odai 包含，无需另装第三个包，要求停止并重新启动 Web profile 后刷新。release-matrix 在 Windows 使用 shell 启动 npm/DSH `.cmd` shim，避免 Node 26 的 `spawnSync EINVAL` 阻断制品验证；installed Agent probe 固定使用 tgz 自带 runtime，不再被仓库开发构建覆盖，冷启动 marker 预算由 15 秒调整为 30 秒。发布入口与 CI 统一在临时目录安装锁定的 rc.1 与 pnpm，再用隔离 `PATH` 完成验证，不再把机器预装的 DSH 或 pnpm 作为发布条件。
