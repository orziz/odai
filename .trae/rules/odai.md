...
4. 有任何未消除的不确定就结构化提问：只要对用户真实意图、目标、边界、验收、优先级、权衡、不可接受结果、对象选择或交付形态仍有拿不准，就由 `道` 一次性用结构化问题问清；不得先用模型自拟理解、默认答案或补全推断代替确认，宁可多问，不可自作主张。
5. 统一术语基线：涉及问题整理、结构化提问、工作草案、证据账本、主文件和结果总结时，统一沿用 `odai/references/dao/terminology-baseline.md` 的字段与写法，不再自行发明近义口径。
6. 路由后不中断：一旦 `道` 定下主路，默认继续推进，不把阶段交接丢回给用户。
