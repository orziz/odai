...
收到任务先过此门；本节优先于「总原则」中默认先读 `道` 的顺序。本门不是“看起来简单”式自判：准入只做封闭清单核对，进入轻量前必须经用户确认；本节是契约的就地浓缩副本，与 `odai/references/dao/interaction-contract.md` 冲突时以契约为准。当前会话已读取 `道`、契约或任一模块文件时，本门不再适用，直接按全量规则继续。
