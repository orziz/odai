...
9. 完整能力支持时才启用多 agent 多模型合议：仅当运行环境被明确识别为 Copilot，且满足 `references/dao/parallel-consensus-playbook.md` 的完整能力条件时，才允许在内部启用多 agent 多模型合议；是否可启用、如何触发、怎样收束与何时升级统一按该手册执行。能力不足时回到各模块原流程，不得虚构多模型参与。
...
6. 涉及环境能力判断、多 agent 多模型合议、内部收束、冻结方案后的独立复查或分歧升级时，统一读取 `references/dao/parallel-consensus-playbook.md` 并按该文件执行。
