...
8. 涉及增强模式、辅助复核、模型派位或冻结方案后的独立复查时，先按 `references/dao/parallel-consensus-trigger.md` 做短判。短判结果为“原流程”时直接继续，不读完整 playbook。只有用户显式要求增强模式、短判裁决为“完整多模型合议”且能力确认可用、或冻结后需独立复查且短判确认值得时，才读 `references/dao/parallel-consensus-playbook.md`；当前已确认可分配模型且需要给席位选模时，再读 `references/dao/model-selection-baseline.md`。
