...
- 或主流程依契约「高代价决策的独立挑战」的征询式升档提议 N 席、且用户同意——等同激活，可进入本模式。
- 可顺带指定席位数与模型，如「合议 4 席 用 gemini ds gpt glm」；未指定时用下面的默认配置。
- 非显式激活、且未经上述征询同意，一律不进入本模式；不得因任务“看起来重要”就自动开（征询式升档仍是提议 + 用户拍板，不是自动开）。其余自动场景由契约「高代价决策的独立挑战」的 1 席兜底覆盖。
...
- 约束：沿用同版 `odai` 契约，含「子 agent 通用边界」（遇缺口停手回交、不自向用户提问、不派生子 agent、主流程复核才采纳）
- 输出格式与收口要求
