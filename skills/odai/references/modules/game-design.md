...
多条、混合或低结构输入需要拆解与反向映射时，读 `references/feature-plan/planning-playbook.md`；本模块只补游戏视觉口径。
...
1. 当前交付相关的玩家 / 平台、视觉目标、现有视觉基线、资产约束或验收口径未清时，不输出正式视觉方案；未涉及的视觉工种不强制补齐。
2. 感知型验收按 `references/dao/interaction-contract.md` 稳定；本模块只补玩家 / 平台、视觉目标、参考、资产约束、取舍和不可接受结果。客观事实可分开报告，不自动变成已选视觉修法。
...
只有感知验收已经稳定、当前确实进入视觉设计交付时，才按下列条件读取；未稳停手轮不得读取。
