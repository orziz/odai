...
4. 正文只写本模块运行真规则；入口已定之提问、确认、术语、记忆、静默、宿主能力与全局边界，引用代替，不重抄。`SKILL.md` 可保留默认触发所需最小摘要；其余 support files 只补本层新增规则，不把入口摘要或基线细则重写第二遍。全局字段/答语/工具前导/能力呈现归 `references/dao/terminology-baseline.md`；阶段流转/执行静默/清单状态源归 `references/harness-dev/workflow-kit.md`。
