...
1. canonical 路径默认是 skills/odai/references/modules/<module-name>.md；第一版即应落真实源文件，不只停于聊天草稿。
...
5. references/<module-name>/ 存长规则；assets/<module-name>/ 存模板；scripts/<module-name>/ 仅在脚本真为执行依据时设。
...
3. 可用结构化提问时必用之；不可用，则先明言，再改文字问答；得答后径行。
