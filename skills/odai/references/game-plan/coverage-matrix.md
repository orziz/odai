...
`game-plan` 覆盖的不是少数题材词，而是常见游戏结构。至少含：
...
3. 若用户只说类型名，不足以直落方案；还须补核心乐趣、目标玩家、平台、版本节奏与商业边界。
...
若题目仍属上述工种，但末了还要落界面、角色场景、品牌包装或特效表现，先由 `game-plan` 定口径，再把视觉层切给 `game-design`。
